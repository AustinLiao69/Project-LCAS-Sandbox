...
### 6.1 核心資料流
- Flutter APP 發起 HTTP Request 到 AP Layer。
- AP Layer 接收 Request，解析參數，並根據 API 設計進行業務邏輯處理。
- AP Layer 透過定義好的介面與 Business Layer 進行交互，傳遞資料與指令。
- Business Layer 處理核心商業邏輯，並將結果回傳給 AP Layer。
- AP Layer 根據 Business Layer 的結果，封裝成標準化的 API Response。
- AP Layer 將 Response 回傳給 Flutter APP。

### 6.2 資料流中的四模式支援
- 在 AP Layer 接收 Request 時，解析 `X-User-Mode` Header。
- 在處理 Business Layer 回傳的資料時，根據 User Mode 進行資料的篩選、轉換或補充。
- 在封裝 API Response 時，確保資料結構與詳細程度符合當前 User Mode 的要求。

### 6.3 錯誤處理流程
- API Request 發生錯誤（如驗證失敗、參數錯誤），AP Layer 直接回傳 4xx 錯誤碼與標準錯誤格式。
- Business Layer 處理邏輯時發生錯誤，會將錯誤資訊傳遞給 AP Layer。
- AP Layer 將 Business Layer 的錯誤資訊，轉換為標準化的 5xx 錯誤碼與錯誤格式，並回傳給 Flutter APP。
- 系統級別的錯誤（如資料庫連接失敗），AP Layer 記錄日誌並回傳 5xx 錯誤碼。