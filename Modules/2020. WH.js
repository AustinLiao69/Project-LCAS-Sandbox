...
function doPost(req, res) {
  ...
  try {
    ...
    if (needsReply) {
      ...
      try {
        // 直接調用同步版本的處理函數
        processWebhookSync({ requestId: requestId, body: req.body });
      } catch (syncError) {
        console.log("同步處理異常:", syncError);
        WH_directLogWrite([
          WH_formatDateTime(new Date()),
          `WH 2.0.16: 同步處理異常: ${syncError.toString()} [${requestId}]`,
          "同步處理",
          "",
          "SYNC_ERROR",
          "WH",
...
/**
 * 02b. 同步處理Webhook請求 - 確保 replyToken 有效性
 * @version 2025-07-11-V2.0.17
 * @date 2025-07-11 12:20:00
 * @update: 新增同步處理函數以確保 replyToken 在有效期內使用
 * @param {Object} e - 觸發器事件對象，包含requestId和body
 */
function processWebhookSync(e) {
  // 從參數獲取請求ID和請求體
  const requestId = e && e.requestId ? e.requestId : "unknown";
  const postData = e && e.body ? e.body : null;
      ...
      for (const event of postData.events) {
        try {
          ...
          if (event.type === "message") {
            // 同步處理消息事件 - 關鍵：不使用 await
            WH_processEventSync(event, requestId, userId);
          } else {
            // 記錄其他類型事件
            WH_directLogWrite([
              WH_formatDateTime(new Date()),
              `WH 2.0.16: 收到${event.type}事件 [${requestId}]`,
              "事件處理",
              userId,
              "",
              "WH",
...
/**
 * 07b. 同步處理事件 - 確保 replyToken 有效性
 * @version 2025-07-11-V2.0.17
 * @date 2025-07-11 12:20:00
 * @update: 新增同步事件處理函數以確保 replyToken 在有效期內使用
 * @param {Object} event - LINE事件對象
 * @param {string} requestId - 請求ID
 * @param {string} userId - 用戶ID
 */
function WH_processEventSync(event, requestId, userId) {
  // 檢查基本參數
  if (!event || !event.type) {
    console.log(`無效事件或缺少類型: ${JSON.stringify(event)} [${requestId}]`);
    WH_directLogWrite([
      WH_formatDateTime(new Date()),
      `WH 2.0.16: 無效事件或缺少類型 [${requestId}]`,
      "事件處理",
      userId,
      "INVALID_EVENT",
      "WH",
  ...
  try {
    ...
    WH_directLogWrite([
      WH_formatDateTime(new Date()),
      `WH 2.0.16: 開始同步處理事件: ${event.type} [${requestId}]`,
      "事件處理",
      userId,
      "",
      "WH",
    ...
    if (event.type === "message") {
      ...
      if (event.message.type === "text") {
        ...
        WH_directLogWrite([
          WH_formatDateTime(new Date()),
          `WH 2.0.16: 收到文本消息: "${text.substr(0, 50)}${text.length > 50 ? "..." : ""}" [${requestId}]`,
          "訊息接收",
          userId,
          "",
          "WH",
        ...
        try {
          ...
          WH_directLogWrite([
            WH_formatDateTime(new Date()),
            `WH 2.0.16: 準備調用DD_distributeData [${requestId}]`,
            "數據分發",
            userId,
            "",
            "WH",
          ...
          // 關鍵：同步調用DD_distributeData
          DD.DD_distributeData(messageData, "LINE", 0).then(result => {
            ...
            if (!result) {
              ...
              WH_directLogWrite([
                WH_formatDateTime(new Date()),
                `WH 2.0.16: DD_distributeData返回空結果 [${requestId}]`,
                "處理異常",
                userId,
                "EMPTY_RESULT",
                "WH",
            ...
            WH_directLogWrite([
              WH_formatDateTime(new Date()),
              `WH 2.0.16: 捕獲DD_distributeData異常，生成預設回覆 [${requestId}]`,
              "異常處理",
              userId,
              "DD_ERROR",
              "WH",
          ...
          // 同步調用異常處理
          console.log(
            `DD_distributeData同步調用失敗: ${ddError.toString()} [${requestId}]`,
          );
        ...
        WH_directLogWrite([
          WH_formatDateTime(new Date()),
          `WH 2.0.16: 收到其他類型消息: ${event.message.type} [${requestId}]`,
          "事件處理",
          userId,
          "",
          "WH",
      ...
      WH_directLogWrite([
        WH_formatDateTime(new Date()),
        `WH 2.0.16: 收到非消息事件: ${event.type} [${requestId}]`,
        "事件處理",
        userId,
        "",
        "WH",
      ...
      if (event.type === "follow") {
        ...
        try {
          ...
          // 調用AM模組建立LINE帳號
          AM.AM_createLineAccount(userId, null, 'J').then(createResult => {
            if (createResult.success) {
              ...
              WH_directLogWrite([
                WH_formatDateTime(new Date()),
                `WH 2.0.16: 用戶關注事件 - 成功建立帳號 ${userId} [${requestId}]`,
                "用戶關注",
                userId,
                "",
                "WH",
              ...
              WH_directLogWrite([
                WH_formatDateTime(new Date()),
                `WH 2.0.16: 用戶關注事件 - 帳號建立失敗 ${userId}: ${createResult.error} [${requestId}]`,
                "用戶關注",
                userId,
            ...
            WH_directLogWrite([
              WH_formatDateTime(new Date()),
              `WH 2.0.16: 處理用戶關注事件錯誤 ${userId}: ${followError.toString()} [${requestId}]`,
              "用戶關注",
              userId,
              "FOLLOW_EVENT_ERROR",
              "WH",
    ...
    WH_directLogWrite([
      WH_formatDateTime(new Date()),
      `WH 2.0.16: 事件處理錯誤: ${error.toString()} [${requestId}]`,
      "事件處理",
      userId,
      "EVENT_ERROR",
      "WH",
