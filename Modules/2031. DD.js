...
try {
  ...
  console.log(`BK_processBookkeeping函數檢查: ${typeof BK_processBookkeeping === 'function' ? "存在" : "不存在"}`);
} catch (error) {
 ...
 * @returns {object} - 處理結果
 */
function DD_distributeData(data, source, retryCount = 0) {
  // 模組初始化檢查
  try {
  ...
  console.log(`DD_distributeData被調用，數據源: ${source}, 用戶ID: ${data.user_id || data.userId || "未知"}, 時間: ${new Date().toISOString()}`);

  const processId = Utilities.getUuid().substring(0, 8);
  console.log(`處理ID: ${processId}`);
  DD_logInfo(`開始處理數據 [${processId}]`, "數據分配", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");
  ...
  try {
    ...
    DD_logDebug(`處理數據: ${dataPreview}, 來源: ${source}`, "數據接收", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");

    // 處理時間戳（如果存在）
    if (data && data.timestamp) {
      console.log(`處理時間戳: ${data.timestamp}`);
      DD_logDebug(`處理時間戳: ${data.timestamp}`, "數據處理", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");
      ...
      if (convertedTime) {
        ...
        console.log(`時間戳轉換結果: ${data.convertedDate} ${data.convertedTime}`);
        DD_logDebug(`時間戳轉換結果: ${data.convertedDate} ${data.convertedTime}`, "數據處理", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");
      } else {
        console.log(`警告: 時間戳轉換失敗: ${data.timestamp}`);
        DD_logWarning(`時間戳轉換失敗: ${data.timestamp}`, "數據處理", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");
      }
    }
    ...
    if (source === '使用者訊息' && data && data.text) {
      ...
      DD_logInfo(`處理用戶訊息: "${data.text}"`, "訊息處理", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");

      const processedData = DD_processUserMessage(data.text, data.userId || data.user_id, data.timestamp);
      console.log(`DD_processUserMessage返回: ${JSON.stringify(processedData)}`);

      if (processedData && processedData.processed) {
        // 保留原始資料，並添加處理後的資訊
        console.log(`成功解析訊息: 科目=${processedData.subjectName}, 金額=${processedData.amount}, 支付方式=${processedData.paymentMethod || "預設"}`);
        DD_logInfo(`成功解析訊息: 科目=${processedData.subjectName}, 金額=${processedData.amount}, 支付方式=${processedData.paymentMethod || "預設"}`, "訊息處理", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");
        ...
        console.log(`訊息解析成功: 科目=${processedData.subjectName}, 金額=${processedData.amount}, 支付方式=${data.paymentMethod || "預設"}`);
      } else if (processedData && processedData.errorMessage) {
        // 處理失敗但有錯誤訊息，直接返回錯誤訊息
        console.log(`訊息解析失敗但有錯誤訊息: ${processedData.errorMessage}`);
        DD_logWarning(`訊息解析失敗但有錯誤訊息: ${processedData.errorMessage}`, "訊息處理", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");
        ...
        // 處理失敗的情況
        console.log(`訊息解析失敗: ${processedData ? processedData.reason : '未知原因'}`);
        DD_logWarning(`訊息解析失敗: ${processedData ? processedData.reason : '未知原因'}`, "訊息處理", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");
    ...
    // 6. 根據數據屬性進行分類
    console.log(`開始分類數據`);
    DD_logInfo(`開始分類數據`, "數據分類", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");

    const category = DD_classifyData(data, source);
    console.log(`數據分類結果: ${category}`);
    DD_logInfo(`數據分類結果: ${category}`, "數據分類", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");

    // 7. 根據分類結果分發數據
    console.log(`開始分發數據至 ${category}`);
    DD_logInfo(`開始分發數據至 ${category}`, "數據分發", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");

    const dispatchResult = DD_dispatchData(data, category);
    console.log(`數據分發完成，結果: ${JSON.stringify(dispatchResult)}`);
    DD_logInfo(`數據分發完成，結果: ${JSON.stringify(dispatchResult)}`, "數據分發", data.user_id || data.userId || "", "DD_distributeData", "DD_distributeData");

    // 使用統一消息格式化處理結果
    return DD_formatSystemReplyMessage(dispatchResult || { 
      success: true, 
      category: category, 
      processId: processId 
    ...
    // 記錄原始錯誤
    const userId = data && (data.user_id || data.userId) ? (data.user_id || data.userId) : "";
    console.log(`數據處理錯誤: ${error.toString()}`);
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`數據處理錯誤: ${error.toString()}`, "數據處理", userId, "ERROR", error.toString(), "DD_distributeData", "DD_distributeData");

    // 8. 重試機制
    if (retryCount < DD_MAX_RETRIES) {
      // 使用指數退避策略延遲重試
      const delayTime = DD_RETRY_DELAY * Math.pow(2, retryCount);
      console.log(`準備重試 (${retryCount+1}/${DD_MAX_RETRIES})，延遲 ${delayTime}ms`);
      DD_logWarning(`準備重試 (${retryCount+1}/${DD_MAX_RETRIES})，延遲 ${delayTime}ms`, "錯誤重試", userId, "DD_distributeData", "DD_distributeData");
...
function DD_classifyData(data, source) {
  ...
  DD_logDebug(`開始分類，來源: ${source} [${classifyId}]`, "數據分類", data.user_id || data.userId || "", "DD_classifyData", "DD_classifyData");
  ...
  try {
    ...
    if (source === 'Rich menu' && data) {
      if (data.action === '記帳' || data.postback === '記帳') {
        ...
        DD_logDebug(`檢測到記帳按鈕操作`, "數據分類", data.user_id || data.userId || "", "DD_classifyData", "DD_classifyData");
      }
    }
    // 規則 2：處理來自使用者訊息
      ...
      if (data.type === '記帳' && data.processed) {
        ...
        DD_logDebug(`檢測到已處理的記帳訊息`, "數據分類", data.user_id || data.userId || "", "DD_classifyData", "DD_classifyData");
      }
    }
    // 規則 3：來自 Webhook 的請求
      ...
      DD_logDebug(`檢測到Webhook請求`, "數據分類", data.user_id || data.userId || "", "DD_classifyData", "DD_classifyData");
    }

    console.log(`分類結果: ${category} [${classifyId}]`);
    DD_logDebug(`分類結果: ${category} [${classifyId}]`, "數據分類", data.user_id || data.userId || "", "DD_classifyData", "DD_classifyData");
    return category;
  } catch (error) {
    console.log(`分類過程出錯: ${error.toString()} [${classifyId}]`);
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`分類過程出錯: ${error.toString()}`, "數據分類", data.user_id || data.userId || "", "ERROR", error.toString(), "DD_classifyData", "DD_classifyData");
    return "ERROR";
  }
}
...
function DD_dispatchData(data, targetModule) {
  ...
  DD_logInfo(`開始分發數據至 ${targetModule} [${dispatchId}]`, "數據分發", userId, "DD_dispatchData", "DD_dispatchData");
  ...
  try {
    ...
    switch (targetModule) {
      case DD_TARGET_MODULE_BK:
        ...
        DD_logInfo(`轉發到BK模組 [${dispatchId}]`, "數據分發", userId, "DD_dispatchData", "DD_dispatchData");

        // 檢查DD_processForBK函數是否存在
        if (typeof DD_processForBK !== 'function') {
          console.log(`DD_processForBK函數不存在 [${dispatchId}]`);
          DD_logError(`DD_processForBK函數不存在 [${dispatchId}]`, "數據分發", userId, "FUNCTION_NOT_FOUND", "函數不存在", "DD_dispatchData");
          ...
          try {
            ...
            console.log(`DD_processForBK調用完成，結果: ${JSON.stringify(result).substring(0, 200)}... [${dispatchId}]`);
          } catch (bkError) {
            // 增強錯誤日誌
            console.log(`調用DD_processForBK時出錯: ${bkError.toString()} [${dispatchId}]`);
            if (bkError.stack) console.log(`錯誤堆疊: ${bkError.stack}`);
            DD_logError(`調用DD_processForBK時出錯: ${bkError.toString()}\n堆疊: ${bkError.stack || 'n/a'}`, 
                      "數據分發", userId, "BK_ERROR", bkError.toString(), "DD_dispatchData");
      ...
      case DD_TARGET_MODULE_WH:
        ...
        DD_logInfo(`轉發到WH模組 [${dispatchId}]`, "數據分發", userId, "DD_dispatchData", "DD_dispatchData");

        // 檢查DD_processForWH函數是否存在
        if (typeof DD_processForWH !== 'function') {
          console.log(`DD_processForWH函數不存在 [${dispatchId}]`);
          DD_logError(`DD_processForWH函數不存在 [${dispatchId}]`, "數據分發", userId, "FUNCTION_NOT_FOUND", "函數不存在", "DD_dispatchData");
          ...
          try {
            ...
            console.log(`DD_processForWH調用完成，結果: ${JSON.stringify(result).substring(0, 200)}... [${dispatchId}]`);
          } catch (whError) {
            // 增強錯誤日誌
            console.log(`調用DD_processForWH時出錯: ${whError.toString()} [${dispatchId}]`);
            if (whError.stack) console.log(`錯誤堆疊: ${whError.stack}`);
            DD_logError(`調用DD_processForWH時出錯: ${whError.toString()}\n堆疊: ${whError.stack || 'n/a'}`, 
                      "數據分發", userId, "WH_ERROR", whError.toString(), "DD_dispatchData");
      ...
      default:
        ...
        DD_logError(`未知的目標模組: ${targetModule} [${dispatchId}]`, "數據分發", userId, "UNKNOWN_MODULE", "模組未知", "DD_dispatchData");
    ...
    console.log(`數據分發完成: ${JSON.stringify(result).substring(0, 200)}... [${dispatchId}]`);
    DD_logInfo(`數據分發完成，結果: ${JSON.stringify(result).substring(0, 200)}... [${dispatchId}]`, "數據分發", userId, "DD_dispatchData", "DD_dispatchData");
    ...
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`分發數據出錯: ${error.toString()}\n堆疊: ${error.stack || 'n/a'}`, 
              "數據分發", userId, "DISPATCH_ERROR", error.toString(), "DD_dispatchData");
...
function DD_processForWH(data) {
  ...
  if (typeof WH_processEvent === 'function') {
    try {
      ...
      console.log(`轉發到WH_processEvent時發生錯誤 [${whId}]: ${error.toString()}`);
...
function DD_processForBK(data) {
  try {
    ...
    // 記錄開始處理
    DD_logInfo(`開始處理記帳數據 [${processId}]`, "記帳處理", userId, "DD_processForBK", "DD_processForBK");

    // 檢查必要字段
    if (!data.action) {
      DD_logWarning(`缺少必要字段: action [${processId}]`, "數據驗證", userId, "DD_processForBK", "DD_processForBK");
      throw new Error("缺少必要字段: action");
    }
    if (!data.subjectName) {
      DD_logWarning(`缺少必要字段: subjectName [${processId}]`, "數據驗證", userId, "DD_processForBK", "DD_processForBK");
      throw new Error("缺少必要字段: subjectName");
    }
    if (!data.amount || data.amount <= 0) {
      DD_logWarning(`無效的金額: ${data.amount} [${processId}]`, "數據驗證", userId, "DD_processForBK", "DD_processForBK");
      throw new Error("無效的金額");
    }
    if (!data.majorCode) {
      DD_logWarning(`缺少必要字段: majorCode [${processId}]`, "數據驗證", userId, "DD_processForBK", "DD_processForBK");
      throw new Error("缺少必要字段: majorCode");
    }
    if (!data.subCode) {
      DD_logWarning(`缺少必要字段: subCode [${processId}]`, "數據驗證", userId, "DD_processForBK", "DD_processForBK");
    ...
    `, "調用準備", userId, "DD_processForBK", "DD_processForBK");

    // 調用BK_processBookkeeping處理記帳
    DD_logInfo(`開始調用BK_processBookkeeping [${processId}]`, "模組調用", userId, "DD_processForBK", "DD_processForBK");
    const result = BK_processBookkeeping(bookkeepingData);
    DD_logInfo(`BK_processBookkeeping調用完成，結果: ${result.success ? "成功" : "失敗"} [${processId}]`, "模組調用", userId, "DD_processForBK", "DD_processForBK");
    ...
    if (result.success) {
      ...
      // 記錄成功訊息
      DD_logInfo(`記帳成功: ID=${result.data.id}, 金額=${bookkeepingData.amount}, 科目=${bookkeepingData.subjectName}, 使用者類型=${result.data.userType || userType || "未知"} [${processId}]`, "記帳結果", userId, "DD_processForBK", "DD_processForBK");
    } else {
      // 失敗回覆
      responseMessage = `記帳失敗！\n原因：${result.error || result.message}\n請重新嘗試或聯繫管理員。`;

      // 記錄失敗訊息
      DD_logWarning(`記帳失敗: ${result.error || result.message} [${processId}]`, "記帳結果", userId, "DD_processForBK", "DD_processForBK");
    }

    DD_logInfo(`生成回覆訊息: ${responseMessage.substring(0, 50)}...`, "訊息生成", userId, "DD_processForBK", "DD_processForBK");
    ...
    // 記錄錯誤
    DD_logError(`處理BK數據時出錯: ${error}`, "數據處理", userId || "", "BK_PROCESS_ERROR", error.toString(), "DD_processForBK", "DD_processForBK");
...
/**
 * 15. 處理用戶消息並提取記帳信息 - 修正收入支出判断逻辑
 * @version 2025-06-17-V9.0.0
 * @author AustinLiao69
 * @date 2025-06-17 09:45:32
 * @update: 修正收入支出判断逻辑、移除支付方式默认值处理
...
function DD_processUserMessage(message, userId = "", timestamp = "") {
  ...
  try {
    // 3. 確保配置初始化
    DD_initConfig();
    DD_logDebug(`DD_CONFIG.SYNONYM檢查: FUZZY_MATCH_THRESHOLD=${DD_CONFIG.SYNONYM && DD_CONFIG.SYNONYM.FUZZY_MATCH_THRESHOLD || "未設置"}, ENABLE_COMPOUND_WORDS=${DD_CONFIG.SYNONYM && DD_CONFIG.SYNONYM.ENABLE_COMPOUND_WORDS || "未設置"}`, "配置檢查", userId, "DD_processUserMessage", "DD_processUserMessage");
    ...
    console.log(`DD_processUserMessage: DD_parseInputFormat返回結果: ${JSON.stringify(parseResult)} [${msgId}]`);

    // 7. 檢查解析結果 - 快速攔截錯誤
    if (!parseResult) {
      DD_logWarning(`DD_parseInputFormat返回null，無法解析訊息格式: "${message}" [${msgId}]`, "訊息處理", userId, "DD_processUserMessage", "DD_processUserMessage");
      console.log(`DD_processUserMessage: DD_parseInputFormat返回null [${msgId}]`);

      // 7.1 創建標準錯誤數據結構，與BK模組格式相容
      const errorData = {
        success: false,
        error: "無法識別記帳意圖",
    ...
    if (parseResult._formatError || parseResult._missingSubject) {
      ...
      // 8.2 自行構造符合BK模組格式的錯誤數據
      console.log(`DD_processUserMessage: 自行構造errorData [${msgId}]`);
      ...
      const errorData = {
        ...
        partialData: {
          ...
          paymentMethod: parseResult.paymentMethod || "", // 不設默認值
          timestamp: new Date().getTime()
        }
      };
    ...
    const paymentMethod = parseResult.paymentMethod; // 直接使用解析結果，不設默認值
    ...
    if (subject) {  // 只检查科目是否存在
      ...
      // 11. 準備返回結果
      if (subjectInfo) {
        console.log(`DD_processUserMessage: 科目匹配完成，準備返回結果 [${msgId}]`);
        DD_logInfo(`科目匹配完成: "${subject}" -> ${subjectInfo.subName} (${matchMethod})`, "科目匹配", userId, "DD_processUserMessage", "DD_processUserMessage");

        // 11.1 決定收支類型 - 修正逻辑!
        let action = "支出"; // 預設為支出

        // 修改：处理负数金额，仍设置为支出但保留负号
        if (amount < 0) {
          action = "支出";
          console.log(`DD_processUserMessage: 檢測到負數金額: ${amount}，設置為支出類型 [${msgId}]`);
          DD_logInfo(`檢測到負數金額: ${amount}，設置為支出類型`, "金額處理", userId, "DD_processUserMessage", "DD_processUserMessage");
        } 
        else {
          // 根據科目大類判斷收支類型 - 修正：以8开头的为收入，其他为支出
        ...
        const result = {
          ...
          amount: amount,
          rawAmount: rawAmount,
          paymentMethod: paymentMethod, // 直接傳遞原始值，不設默認值
          action: action,
          confidence: confidence,
          matchMethod: matchMethod,
          text: remarkText, // 移除金額後的文本作為備註
        ...
        console.log(`DD_processUserMessage: 返回結果: ${JSON.stringify(result)} [${msgId}]`);
        return result;
      } else {
        // 11.3 科目匹配失敗處理
        ...
        const errorData = {
          ...
          partialData: {
            ...
            amount: amount,
            rawAmount: rawAmount,
            paymentMethod: paymentMethod, // 不設默認值
            timestamp: new Date().getTime()
          }
        };
...
function DD_getSubjectCode(subjectName) {
  ...
  try {
    ...
    if (!subjectName) {
      ...
      DD_logWarning(`科目名稱為空，無法查詢科目代碼 [${scId}]`, "科目查詢", "", "DD_getSubjectCode");
      return null;
    }
    ...
    if (!sheet) {
      ...
      DD_logError(`找不到科目表: ${DD_SUBJECT_CODE_SHEET_NAME} [${scId}]`, "科目查詢", "", "SHEET_NOT_FOUND", "找不到科目代碼表", "DD_getSubjectCode");
      return null;
    }
    ...
    if (lastRow <= 1) {
      ...
      DD_logError(`科目表為空或只有標題行 [${scId}]`, "科目查詢", "", "EMPTY_SHEET", "科目代碼表無數據", "DD_getSubjectCode");
      return null;
    }
    ...
    for (let i = 1; i < values.length; i++) {
      ...
      if (i < 10 || normalizedSubName === normalizedInput) {
        console.log(`科目表項目 #${i}: 代碼=${majorCode}-${subCode}, 名稱="${normalizedSubName}"`);
      }
      ...
      if (subNameLower === inputLower) {
        ...
        DD_logInfo(`成功查詢科目代碼: ${majorCode}-${subCode} ${normalizedSubName} [${scId}]`, "科目查詢", "", "DD_getSubjectCode");
      ...
      if (synonymsStr) {
        ...
        for (let j = 0; j < synonyms.length; j++) {
          ...
          if (synonymLower === inputLower) {
            console.log(`通過同義詞匹配成功: "${synonymLower}" === "${inputLower}"`);

            DD_logInfo(`通過同義詞成功查詢科目代碼: ${majorCode}-${subCode} ${normalizedSubName} [${scId}]`, "科目查詢", "", "DD_getSubjectCode");
            console.log(`---科目查詢診斷信息結束---[${scId}]`);
    ...
    for (let i = 1; i < values.length; i++) {
      ...
      if (subNameLower.length >= 2 && inputLower.includes(subNameLower)) {
        ...
        console.log(`複合詞包含科目名: 輸入="${inputLower}" 包含科目="${subNameLower}" 分數=${score.toFixed(2)}`);
        matches.push({
          majorCode: String(majorCode),
          majorName: String(majorName),
          subCode: String(subCode),
      ...
      if (synonymsStr) {
        ...
        for (const syn of synonyms) {
          ...
          if (synonym.length >= 2 && inputLower.includes(synonym)) {
            ...
            console.log(`複合詞包含同義詞: 輸入="${inputLower}" 包含同義詞="${synonym}" 分數=${score.toFixed(2)}`);
            matches.push({
              majorCode: String(majorCode),
              majorName: String(majorName),
              subCode: String(subCode),
          ...
          if (synonym.includes(' ') && synonym.includes(inputLower)) {
            ...
            console.log(`輸入詞是含空格同義詞的一部分: 輸入="${inputLower}" 是同義詞"${synonym}"的一部分, 分數=${score.toFixed(2)}`);
            matches.push({
              majorCode: String(majorCode),
              majorName: String(majorName),
              subCode: String(subCode),
    ...
    if (matches.length > 0) {
      ...
      console.log(`複合詞匹配成功: "${normalizedInput}" -> "${bestMatch.subName}", 分數=${bestMatch.score.toFixed(2)}, 匹配類型=${bestMatch.matchType}`);
      DD_logInfo(`複合詞匹配成功: "${normalizedInput}" -> "${bestMatch.subName}", 分數=${bestMatch.score.toFixed(2)}`, "複合詞匹配", "", "DD_getSubjectCode");
    ...
    DD_logWarning(`科目代碼查詢失敗: "${normalizedInput}" [${scId}]`, "科目查詢", "", "DD_getSubjectCode");
    console.log(`---科目查詢診斷信息結束---[${scId}]`);
    return null;
    ...
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`科目查詢出錯: ${error} [${scId}]`, "科目查詢", "", "QUERY_ERROR", error.toString(), "DD_getSubjectCode");
    return null;
  }
}
...
function DD_logError(message, operationType = "", userId = "", errorCode = "", errorDetails = "", location = "", functionName = "") {
  DD_log("ERROR", message, operationType, userId, { errorCode, errorDetails, location, functionName });
}

function DD_logCritical(message, operationType = "", userId = "", errorCode = "", errorDetails = "", location = "", functionName = "") {
  DD_log("CRITICAL", message, operationType, userId, { errorCode, errorDetails, location, functionName });
}
 ...
 */
function DD_writeToLogSheet(severity, message, operationType, userId, errorCode = "", source = "DD", errorDetails = "", 
  retryCount = 0, location = "", functionName = "") {
  try {
    // 直接寫入日誌，不依賴其他模組
    ...
    if (logSheet) {
      // 使用台灣時區格式化時間戳記
      const timestamp = Utilities.formatDate(new Date(), DD_CONFIG.TIMEZONE, "yyyy-MM-dd HH:mm:ss");
      ...
      console.log(`找不到名為 '${DD_CONFIG.LOG_SHEET_NAME}' 的工作表，無法寫入日誌。`);
    }
  } catch (error) {
    // 如果寫入日誌失敗，只能在控制台輸出
...
function DD_removeAmountFromText(text, amount, paymentMethod) {
  ...
  // 記錄處理前文字
  console.log(`處理文字移除金額和支付方式: 原始文字="${text}", 金額=${amount}, 支付方式=${paymentMethod || "未指定"}`);
...
function DD_fuzzyMatch(input, threshold = 0.6) {
  ...
  allSubjects.forEach(subject => {
    ...
    if (subNameLower.length >= 2 && inputLower.includes(subNameLower)) {
      ...
      console.log(`【模糊匹配】輸入包含科目名: ${inputLower} 包含 ${subNameLower}, 分數=${score.toFixed(2)}`);
    }
    ...
    if (subject.synonyms) {
      ...
      for (const synonym of synonymsList) {
        ...
        if (synonym.length >= 2 && inputLower.includes(synonym)) {
          ...
          console.log(`【模糊匹配】輸入包含同義詞: ${inputLower} 包含 ${synonym}, 分數=${score.toFixed(2)}`);
        }
      }
    }
  });
  ...
  if (containsMatches.length > 0) {
    ...
    console.log(`【模糊匹配】複合詞最佳匹配: "${input}" -> "${bestMatch.subName}", 包含詞: "${bestMatch.matchedTerm}", 分數: ${bestMatch.score.toFixed(2)}`);
  ...
  allSubjects.forEach(subject => {
    ...
    // 檢查同義詞
    if (subject.synonyms) {
      const synonymsList = subject.synonyms.split(',').map(s => s.trim().toLowerCase());
  ...
  if (matches.length > 0) {
    ...
    console.log(`【模糊匹配】標準匹配成功: "${input}" -> "${bestMatch.subName}" (分數: ${bestMatch.score}, 類型: ${bestMatch.matchType})`);
    return bestMatch;
  }
...
function DD_timeAwareClassification(possibleMatches, timestamp) {
  ...
  console.log(`開始時間感知分類，有 ${possibleMatches ? possibleMatches.length : 0} 個可能匹配 [${tacId}]`);
  ...
  try {
    ...
    // 時段定義
    const timeRanges = {
      breakfast: { range: [5, 10], names: ["早餐", "早點", "早午餐"], priority: 0.9 },
      lunch: { range: [11, 14], names: ["午餐", "中餐", "便當", "午飯"], priority: 0.9 },
      dinner: { range: [17, 21], names: ["晚餐", "晚飯", "宵夜"], priority: 0.9 },
      midnight: { range: [22, 4], names: ["宵夜", "消夜", "夜宵"], priority: 0.8 }
    };
    ...
    if (currentTimeSlot) {
      ...
      for (const match of possibleMatches) {
        ...
        // 檢查科目名稱是否包含當前時段的關鍵字
        if (matchNames.some(keyword => subject.includes(keyword.toLowerCase()))) {
          console.log(`找到時段匹配: ${match.subName} 匹配時段 ${currentTimeSlot} [${tacId}]`);
          DD_logInfo(`時間感知分類: "${match.subName}" 匹配當前時段 ${currentTimeSlot}`, "時間感知", "", "DD_timeAwareClassification");
          return { 
            ...match, 
    ...
    console.log(`無時段匹配結果，使用第一個匹配: ${possibleMatches[0].subName} [${tacId}]`);
    DD_logDebug(`無時段匹配結果，使用第一個匹配: ${possibleMatches[0].subName}`, "時間感知", "", "DD_timeAwareClassification");
    return { ...possibleMatches[0], confidence: 0.7 };
  } catch (error) {
    console.log(`時間感知分類錯誤: ${error} [${tacId}]`);
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`時間感知分類錯誤: ${error}`, "同義詞處理", "", "TIME_CLASS_ERROR", error.toString(), "DD_timeAwareClassification");
...
function DD_checkMultipleMapping(term) {
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const synonyms = (data[i][DD_SUBJECT_CODE_SYNONYMS_COLUMN - 1] || "").split(",").map(s => s.trim());
    ...
    if (matches.length > 0) {
      ...
      DD_logInfo(`詞彙 "${term}" 有 ${matches.length} 個映射`, "多重映射", "", "DD_checkMultipleMapping");
      return matches;
    } else {
    ...
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`檢查多重映射錯誤: ${error}`, "同義詞處理", "", "MULTI_MAP_ERROR", error.toString(), "DD_checkMultipleMapping");
    return null;
  }
}
 ...
 */
function DD_userPreferenceManager(userId, inputTerm, selectedSubjectCode, isQuery = false) {
  const upId = Utilities.getUuid().substring(0, 8);
  console.log(`${isQuery ? "查詢" : "存儲"}用戶偏好: userId=${userId}, term="${inputTerm}" [${upId}]`);
  ...
  try {
    ...
    if (!prefSheet) {
      ...
      prefSheet.appendRow(["userId", "inputText", "selectedCategory", "count", "lastUse", "context"]);
    }
    ...
    if (isQuery) {
      ...
      if (matches.length > 0) {
        ...
        console.log(`找到用戶偏好: ${matches[0].subjectCode}, 使用次數=${matches[0].count} [${upId}]`);
        DD_logInfo(`找到用戶偏好: "${inputTerm}" -> ${matches[0].subjectCode}, 使用次數=${matches[0].count}`, "用戶偏好", userId, "DD_userPreferenceManager");
        return matches[0];
      }
    ...
    // 存儲模式
      ...
      if (found) {
        ...
        console.log(`更新用戶偏好: "${inputTerm}" -> ${selectedSubjectCode}, 新計數=${currentCount + 1} [${upId}]`);
        DD_logInfo(`更新用戶偏好: "${inputTerm}" -> ${selectedSubjectCode}, 新計數=${currentCount + 1}`, "用戶偏好", userId, "DD_userPreferenceManager");
      } else {
        // 添加新記錄
        prefSheet.appendRow([userId, inputTerm, selectedSubjectCode, 1, now, ""]);
        console.log(`新增用戶偏好: "${inputTerm}" -> ${selectedSubjectCode} [${upId}]`);
        DD_logInfo(`新增用戶偏好: "${inputTerm}" -> ${selectedSubjectCode}`, "用戶偏好", userId, "DD_userPreferenceManager");
      }
    ...
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`用戶偏好管理錯誤: ${error}`, "同義詞處理", userId, "USER_PREF_ERROR", error.toString(), "DD_userPreferenceManager");
    return null;
  }
}
...
function DD_learnSynonym(term, subjectCode, userId) {
  ...
  console.log(`學習同義詞: "${term}" -> ${subjectCode}, userId=${userId} [${lsId}]`);
  ...
  try {
    ...
    // 尋找對應的科目
    for (let i = 1; i < data.length; i++) {
      const rowMajorCode = String(data[i][DD_SUBJECT_CODE_MAJOR_CODE_COLUMN - 1]);
      const rowSubCode = String(data[i][DD_SUBJECT_CODE_SUB_CODE_COLUMN - 1]);

      if (rowMajorCode === majorCode && rowSubCode === subCode) {
        // 找到科目，處理同義詞
        const currentSynonyms = data[i][DD_SUBJECT_CODE_SYNONYMS_COLUMN - 1] || "";
        const synonymsList = currentSynonyms.split(",")
                            .map(s => s.trim())
                            .filter(s => s.length > 0);

        // 檢查同義詞是否已存在
        if (synonymsList.some(syn => syn.toLowerCase() === term.toLowerCase().trim())) {
        ...
        // 更新科目表
        sheet.getRange(i+1, DD_SUBJECT_CODE_SYNONYMS_COLUMN).setValue(newSynonyms);
        console.log(`成功添加同義詞: "${term}" -> ${subjectCode} [${lsId}]`);
        DD_logInfo(`成功添加同義詞: "${term}" -> ${subjectCode}`, "同義詞學習", userId, "DD_learnSynonym");
        return true;
      }
    }
    ...
    DD_logWarning(`找不到對應科目代碼: ${subjectCode}`, "同義詞學習", userId, "DD_learnSynonym");
    return false;

  } catch (error) {
    console.log(`同義詞學習錯誤: ${error} [${lsId}]`);
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`同義詞學習錯誤: ${error}`, "同義詞處理", userId, "SYN_LEARN_ERROR", error.toString(), "DD_learnSynonym");
    return false;
  }
}
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (rowMajorCode === majorCode && rowSubCode === subCode) {
        ...
        const synonyms = synonymsStr.split(",").map(s => s.trim().toLowerCase());

        const isInSynonyms = synonyms.includes(normalizedTerm);
        console.log(`"${term}" ${isInSynonyms ? "已是" : "不是"} ${subjectCode} 的同義詞 [${csId}]`);
        return isInSynonyms;
      }
    }
    ...
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`檢查同義詞錯誤: ${error}`, "同義詞處理", "", "CHECK_SYN_ERROR", error.toString(), "DD_checkSynonym");
    return false;
  }
}
...
function DD_initConfig() {
  ...
  DD_CONFIG.SPREADSHEET_ID = DD_CONFIG.SPREADSHEET_ID || "1fYFPjswEF0jOEj4TSDehJPNTwBEVwv666jqnN2KMOKU";
  ...
  if (!DD_CONFIG.SYNONYM) {
    ...
    DD_CONFIG.SYNONYM.FUZZY_MATCH_THRESHOLD = DD_CONFIG.SYNONYM.FUZZY_MATCH_THRESHOLD || 0.7;
    DD_CONFIG.SYNONYM.ENABLE_COMPOUND_WORDS = DD_CONFIG.SYNONYM.ENABLE_COMPOUND_WORDS !== undefined ? 
                                             DD_CONFIG.SYNONYM.ENABLE_COMPOUND_WORDS : true;
  }
...
function normalizeChineseInput(input) {
  ...
  for (let i = 0; i < fullWidthNums.length; i++) {
    result = result.replace(new RegExp(fullWidthNums[i], 'g'), halfWidthNums[i]);
  }
 ...
 */
function DD_synonymLearning(userId, originalSubject, matchedSubject, subjectCode) {
  const lsId = Utilities.getUuid().substring(0, 8);
  console.log(`【同義詞學習】開始處理: 用戶="${userId}", 原詞="${originalSubject}", 科目="${matchedSubject}", 代碼=${subjectCode} [${lsId}]`);
  ...
  try {
    ...
    if (DD_checkSynonym(normalizedInput, subjectCode)) {
      console.log(`【同義詞學習】"${originalSubject}"已經是科目${subjectCode}的同義詞，無需重複學習 [${lsId}]`);
    ...
    console.log(`【同義詞學習】當前科目同義詞: ${currentSynonyms || "無"} [${lsId}]`);

    // 2. 從Test ledger中抓取可能的新同義詞
    const ledgerSynonyms = DD_fetchSynonymsFromLedger(subjectCode);
    console.log(`【同義詞學習】從Ledger獲取的同義詞: ${ledgerSynonyms || "無"} [${lsId}]`);
    ...
    console.log(`【同義詞學習】更新後的同義詞列表: ${updatedSynonyms} [${lsId}]`);

    // 4. 更新科目代碼表
    const updateResult = DD_updateSynonymsForSubject(subjectCode, updatedSynonyms);

    if (updateResult.success) {
      console.log(`【同義詞學習】同義詞更新成功 [${lsId}]`);
      DD_logInfo(`同義詞學習成功: "${originalSubject}" -> ${matchedSubject} (${subjectCode})`, "同義詞學習", userId, "DD_synonymLearning");
      ...
      console.log(`【同義詞學習】同義詞更新失敗: ${updateResult.error} [${lsId}]`);

      // 使用56號函數處理錯誤
      const updateErrorResult = DD_formatUserErrorFeedback(updateResult.error, "DD", {
    ...
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`同義詞學習錯誤: ${error}`, "同義詞處理", userId, "SYN_LEARN_ERROR", error.toString(), "DD_synonymLearning");
...
function DD_fetchSynonymsFromLedger(subjectCode) {
  ...
  console.log(`【抓取同義詞】開始從Test ledger抓取科目${subjectCode}的同義詞 [${flId}]`);
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      // 檢查是否匹配科目代碼
      if (String(data[i][MAJOR_CODE_COL]) === majorCode && 
    ...
    console.log(`【抓取同義詞】從Test ledger找到${synonyms.size}個同義詞: ${result} [${flId}]`);
    return result;

  } catch (error) {
    console.log(`【抓取同義詞】處理錯誤: ${error} [${flId}]`);
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`從Ledger抓取同義詞錯誤: ${error}`, "同義詞處理", "", "FETCH_SYN_ERROR", error.toString(), "DD_fetchSynonymsFromLedger");
    return "";
  }
}
...
function DD_getSynonymsForSubject(subjectCode) {
  try {
    ...
    // 尋找匹配的科目
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][DD_SUBJECT_CODE_MAJOR_CODE_COLUMN - 1]) === majorCode && 
    ...
    DD_logError(`獲取科目同義詞錯誤: ${error}`, "同義詞處理", "", "GET_SYN_ERROR", error.toString(), "DD_getSynonymsForSubject");
    return "";
  }
}
...
function DD_updateSynonymsForSubject(subjectCode, synonyms) {
  ...
  console.log(`【更新同義詞】開始更新科目${subjectCode}的同義詞為: ${synonyms} [${usId}]`);
  ...
  try {
    ...
    if (!sheet) {
      console.log(`【更新同義詞】找不到科目表: ${DD_SUBJECT_CODE_SHEET_NAME} [${usId}]`);
    ...
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][DD_SUBJECT_CODE_MAJOR_CODE_COLUMN - 1]) === majorCode && 
        ...
        console.log(`【更新同義詞】成功更新科目${subjectCode}的同義詞 [${usId}]`);
        DD_logInfo(`更新科目${subjectCode}的同義詞: ${synonyms}`, "同義詞管理", "", "DD_updateSynonymsForSubject");
    ...
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`更新科目同義詞錯誤: ${error}`, "同義詞處理", "", "UPDATE_SYN_ERROR", error.toString(), "DD_updateSynonymsForSubject");
...
function DD_generateBookkeepingResponse(result, action) {
  ...
  if (result.success) {
    ...
    const amount = bkResult.income || bkResult.expense || 
                   (isIncome ? result.income : result.expense) || "0";

    // 取得支付方式
    const paymentMethod = bkResult.paymentMethod || result.paymentMethod || "現金";

    // 取得日期時間 (優先使用 BK 結果中的時間)
    const dateStr = bkResult.date || Utilities.formatDate(new Date(), "Asia/Taipei", "yyyy/MM/dd");
    const timeStr = bkResult.time || Utilities.formatDate(new Date(), "Asia/Taipei", "HH:mm");
    ...
    console.log(`生成記帳回覆: 科目=${itemName}, 金額=${amount}, 支付方式=${paymentMethod}`);
...
function DD_generateBookkeepingMessage(data) {
  try {
    ...
    // 2. 檢查必要參數
    if (!data) {
      const errorResult = DD_formatUserErrorFeedback("記帳數據為空，無法生成訊息", "BK", {
    ...
    if (data.success !== false) {
      ...
      // 失敗訊息 - 使用56號函數
      const errorResult = DD_formatUserErrorFeedback(data.error || "未知錯誤", "BK", {
    ...
    // 函數本身出錯時的處理
    const errorResult = DD_formatUserErrorFeedback(`訊息生成出錯: ${error.toString()}`, "DD", {
...
function DD_parseInputFormat(text, processId) {
  ...
  try {
    ...
    if (negativeMatch) {
      ...
      console.log(`DD_parseInputFormat: 識別負數格式 - 科目:「${subject}」, 金額:${rawAmount}, 支付方式:「${paymentMethod}」 [${processId}]`);

      // 負數金額檢查 (關鍵部分：改為在這裡處理錯誤，同時保留原始數據)
      if (amount < 0) {
        console.log(`DD_parseInputFormat: 檢測到負數金額 ${amount} [${processId}]`);
    ...
    if (match) {
      ...
      console.log(`DD_parseInputFormat: 識別標準格式 - 科目:「${subject}」, 金額:${amount}, 支付方式:「${paymentMethod}」 [${processId}]`);
...
function DD_generateIntelligentRemark(bookkeepingData) {
  try {
    ...
    if (bookkeepingData.text && bookkeepingData.text !== bookkeepingData.subjectName) {
      // 構建格式化所需信息
      const parseResult = {
      ...
      // 嘗試格式化
      const formattedRemark = DD_formatBookkeepingRemark(parseResult, bookkeepingData.text);
...
function DD_processParseResult(parseResult, options = {}) {
  ...
  console.log(`[${processId}] DD_processParseResult: 處理基本信息 - 科目: ${subject}, 金額: ${amount}, 原始金額: ${rawAmount}, 支付方式: ${paymentMethod}`);
  ...
  if (parseResult.formatId === "FORMAT8" && options.contextSubject) {
    console.log(`[${processId}] DD_processParseResult: 處理純數字格式，使用上下文科目: ${options.contextSubject}`);
    return {
  ...
  // 6. 返回處理結果
  console.log(`[${processId}] DD_processParseResult: 返回處理結果 - 科目: ${subject}, 金額: ${amount}, 原始金額: ${rawAmount}, 動作: ${action}`);
...
function DD_getAllSubjects() {
  try {
    // 獲取科目資料表
    const sheet = SpreadsheetApp.getActive().getSheetByName("997. 科目代碼_測試");
    if (!sheet) {
...
function DD_getSubjectByCode(subjectCode) {
  ...
  try {
    ...
    // 查詢匹配科目
    for (let i = 1; i < values.length; i++) {
      const currentMajorCode = String(values[i][DD_SUBJECT_CODE_MAJOR_CODE_COLUMN - 1]);
      const currentSubCode = String(values[i][DD_SUBJECT_CODE_SUB_CODE_COLUMN - 1]);

      // 如果只有subCode，找到第一個匹配的子科目
      if (!majorCode && currentSubCode === subCode) {
        console.log(`找到科目: ${currentMajorCode}-${currentSubCode} [${sbcId}]`);
        return {
    ...
    if (error.stack) console.log(`錯誤堆疊: ${error.stack}`);
    DD_logError(`科目代碼查詢出錯: ${error}`, "科目查詢", "", "CODE_QUERY_ERROR", error.toString(), "DD_getSubjectByCode");
    return null;
  }
}