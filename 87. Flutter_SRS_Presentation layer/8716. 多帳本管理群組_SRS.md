...
| **P017** | 建立帳本頁面 | **F010** | POST /app/projects/create | POST | 建立專案帳本 |
| **P017** | 建立帳本頁面 | **F013** | POST /app/categories/create | POST | 建立分類帳本 |
| **P018** | 帳本設定頁面 | **F011** | PUT /app/projects/manage | PUT | 更新專案帳本設定 |
| **P018** | 帳本設定頁面 | **F014** | PUT /app/categories/manage | PUT | 更新分類帳本設定 |
| **P019** | 專案帳本詳情 | **F011** | PUT /app/projects/manage | PUT | 專案帳本管理 |
| **P020** | 分類帳本詳情 | **F014** | PUT /app/categories/manage | PUT | 分類帳本管理 |
| **P021** | 成員管理頁面 | **F011** | PUT /app/projects/manage | PUT | 專案成員管理 |
| **P022** | 權限設定頁面 | **F020** | PUT /app/shared/permissions | PUT | 成員權限設定 |
| **P023** | 邀請成員頁面 | **F019** | POST /app/shared/create | POST | 建立共享帳本 |
| **P024** | 帳本統計頁面 | **F007** | GET /app/ledger/query | GET | 帳本資料統計 |
| **P025** | 刪除帳本頁面 | **F012** | DELETE /app/projects/delete | DELETE | 刪除專案帳本 |

### 3.2 API使用統計
- **覆蓋API數量**：8個多帳本相關API（F007, F010-F015, F019-F020）
- **API使用率**：100%（多帳本功能完整覆蓋）
- **高頻API**：F015、F007（帳本切換與查詢）
- **整合複雜度**：高（跨帳本權限控制）
...
| 版本 | 日期 | 修改者 | 修改內容 |
|------|------|--------|----------|
| **v2.1.0** | **2025-01-27** | **LCAS SA Team** | **🔧 重要修正：API端點對照表統一化修正** |
|  |  |  | **• 修正API編號體系，統一使用F007, F010-F015, F019-F020** |
|  |  |  | **• 對應9005和9205文件的API規格標準** |
|  |  |  | **• 確保前後端整合的一致性** |
|  |  |  | **• 移除重複或無效的API端點對應** |
| **v2.0.0** | **2025-08-07** | **LCAS SA Team** | **🎉 重大版本：多帳本管理群組整合SRS初版建立** |
|  |  |  | **• 整合P016-P025十個帳本管理頁面為統一群組** |
|  |  |  | **• 建立完整的API端點對照表（已於v2.1.0修正）** |
|  |  |  | **• 實作四模式差異化帳本管理體驗設計** |
|  |  |  | **• 建立完整的權限控制與成員管理架構** |
|  |  |  | **• 實作即時協作與衝突解決機制** |
|  |  |  | **• 建立多帳本統計分析與比較系統** |
