...
└── 共用模組群 (Shared Module Group)
    ...
    ├── DL.js - 診斷日誌模組
    ├── GR.js - 通用工具模組
    └── firebase-config.js  - Firebase配置模組
```
...
**功能職責**：
- **記帳交易管理**：記帳資料的建立、修改、刪除、查詢
- **帳戶管理**：帳戶建立、更新、餘額管理、轉帳功能
- **附件管理**：交易附件上傳、刪除
- **批次操作**：批次記帳、匯入交易
- **統計分析**：儀表板數據、圖表數據、最近交易
...
**功能職責**：
- **認證服務**：使用者註冊、登入、登出、Google登入、重設密碼、email驗證
- **跨平台綁定**：LINE綁定、綁定狀態查詢
- **用戶管理**：個人資料管理、偏好設定、安全設定
...
**技術堆疊**：Node.js + Express + Firebase SDK + JWT

#### 4.2.3 MLS.js - 多帳本管理模組 v2.1.0
**功能職責**：
- **帳本管理**：個人帳本和群組帳本管理、帳本CRUD操作
...
**功能職責**：
- **重複交易管理**：定期記帳設定、重複交易CRUD
- **通知管理**：通知設定、通知列表、已讀狀態管理
- **提醒規則**：記帳提醒建立、更新、刪除
- **通知模板**：提醒模板管理
- **自動推播**：財務統計自動推播
...
**技術堆疊**：Node.js + Express + Firebase SDK + Cron
...
**技術堆疊**：Node.js + Express + Firebase SDK + Chart.js
...
**技術堆疊**：Node.js + Express + Firebase SDK + Archive.js
...
**技術堆疊**：Node.js + Firebase SDK

#### 4.3.2 DD2.js - 智慧處理模組 v2.1.0
**功能職責**：
- **AI助理功能**：智慧分類、AI建議、預算建議、異常檢測、支出洞察、個人化推薦
- **激勵系統功能**：成就系統、挑戰機制、積分管理、排行榜、進度追蹤
- **智慧處理**：自動分類、標籤處理、數據分析
- **機器學習**：用戶行為分析、消費模式識別
...
**技術堆疊**：Node.js + Firebase SDK + AI/ML APIs
...
**技術堆疊**：Node.js + Firebase SDK + Redis

#### 4.3.4 FS.js - Firestore操作模組 v1.1.0
**功能職責**：
- **資料庫操作**：Firestore CRUD 操作統一封裝
- **查詢最佳化**：複雜查詢的效能調整
- **交易處理**：資料一致性和原子操作
- **連線管理**：資料庫連線池管理

**API端點對應**：無直接對外API（內部資料操作）

**技術堆疊**：Firebase Admin SDK
...
**技術堆疊**：Node.js + Firebase SDK + Winston
...
**功能職責**：
- **Firebase初始化**：Firebase 服務初始化配置
- **憑證管理**：服務憑證和金鑰管理
- **環境配置**：開發、測試、生產環境配置
- **連線管理**：Firebase連線設定和監控

**API端點對應**：無直接對外API（內部配置）
