...
- 使用者透過 LINE 傳送文字訊息
- LINE 平台透過 Webhook 將訊息 POST 到商業邏輯層中的 LINE OA模組群
- 使用者收到來自後端的回覆訊息

#### 3.2.2 Mobile App（使用 Flutter 開發）
- 使用者可使用APP相關功能
- App 透過 RESTful API 呼叫 商業邏輯層中的 APP模組群以取得或傳送資料

### 3.3 商業邏輯層 (Business Logic Layer)：Replit 模組群

#### 3.3.1 LINE OA模組群（LINE OA module group）
...
#### 3.3.2 APP模組群（APP module group）
- 提供給 App 使用的 RESTful API（如 /api/ledger/add, /api/ledger/getMonthly 等）
- 驗證 Firebase Token 或 JWT，確保身份安全
- 呼叫共用模組群讀寫 Firestore 資料
- 回傳 JSON 結果給前端 App
- **技術堆疊**：Node.js + Express + Firebase SDK

#### 3.3.3 共用模組群（Shared module group）
...
#### 3.4.3 Firebase Cloud Storage（選用）
- 儲存報表檔案（如 PDF、Excel）
