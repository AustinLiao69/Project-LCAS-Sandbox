

# 8801. TDD_P001_歡迎頁面_v2.2.0

## 文件資訊
- **文件標題**: P001 歡迎頁面技術設計文件
- **文件版本**: v2.2.0
- **創建日期**: 2025-08-06 12:00:00 +08:00 (台灣時間)
- **創建者**: LCAS SA Team
- **最後更新**: 2025-08-06 15:00:00 +08:00 (台灣時間)
- **對應需求文件**: 8701. 歡迎頁面_SRS.md
- **對應PRD文件**: 9005. Flutter_Presentation layer.md
- **頁面代碼**: P001 (Welcome Page)

---

## 📑 目次 (Table of Contents)

1. [技術架構概述](#10-技術架構概述)
2. [Widget清單](#20-widget清單)
3. [核心Widget設計](#30-核心widget設計)
4. [狀態管理架構](#40-狀態管理架構)
5. [API整合設計](#50-api整合設計)
6. [導航路由設計](#60-導航路由設計)
7. [響應式設計](#70-響應式設計)
8. [無障礙功能](#80-無障礙功能)
9. [效能最佳化](#90-效能最佳化)
10. [錯誤處理機制](#100-錯誤處理機制)

---

## 1.0 技術架構概述

### 1.1 模組定位
P001歡迎頁面作為LCAS 2.0 Flutter應用程式的入口頁面，負責使用者首次體驗、模式選擇、品牌展示及導航路由分發。本頁面採用MVP原則設計，聚焦核心功能實現。

### 1.2 技術堆疊
- **前端框架**: Flutter 3.x
- **狀態管理**: Provider + ChangeNotifier
- **路由管理**: GoRouter
- **HTTP客戶端**: Dio
- **本地儲存**: SharedPreferences
- **動畫處理**: Flutter內建Animation API
- **無障礙支援**: Flutter Semantics API

### 1.3 模組架構
```dart
/**
 * P001_歡迎頁面_2.2.0
 * @module P001歡迎頁面
 * @description Flutter Presentation Layer入口頁面 - 四模式選擇、品牌展示、導航分發
 * @update 2025-08-06: 升級至v2.2版本，專注技術設計實作，移除測試案例相關內容
 */
```

---

## 2.0 Widget清單

### 2.1 完整Widget索引表

| # | Widget名稱 | 版本 | 所屬層級 | 功能描述 | 相依組件 |
|---|------------|------|----------|----------|----------|
| 01 | WelcomePage | V2.2.0 | 頁面容器層 | 主頁面容器及生命週期管理 | WelcomeProvider、Router |
| 02 | WelcomeScaffold | V2.2.0 | 佈局結構層 | 基礎頁面結構與背景設定 | GradientBackground |
| 03 | BrandSection | V2.2.0 | 品牌展示層 | Logo與品牌標題顯示 | BrandLogo、BrandTitle |
| 04 | ModeSelectionSection | V2.2.0 | 模式選擇層 | 四模式選擇介面 | ModeGrid、ModeCard |
| 05 | NavigationSection | V2.2.0 | 導航操作層 | 開始使用按鈕與導航控制 | ActionButton、LoadingIndicator |
| 06 | BrandLogo | V2.2.0 | 視覺元件層 | 應用程式品牌Logo | Image、AnimatedContainer |
| 07 | BrandTitle | V2.2.0 | 視覺元件層 | 品牌標題與副標題文字 | Text、AnimatedOpacity |
| 08 | ModeGrid | V2.2.0 | 佈局元件層 | 四模式卡片網格佈局 | GridView、ResponsiveWrapper |
| 09 | ModeCard | V2.2.0 | 互動元件層 | 單一模式選擇卡片 | InkWell、AnimatedScale |
| 10 | ActionButton | V2.2.0 | 互動元件層 | 主要操作按鈕（開始使用） | ElevatedButton、AnimatedContainer |
| 11 | GradientBackground | V2.2.0 | 裝飾元件層 | 頁面漸層背景效果 | Container、LinearGradient |
| 12 | LoadingIndicator | V2.2.0 | 狀態元件層 | 載入中指示器 | CircularProgressIndicator |
| 13 | ErrorDialog | V2.2.0 | 錯誤處理層 | 錯誤訊息對話框 | AlertDialog、TextButton |

### 2.2 Widget分層架構

```
P001歡迎頁面 (v2.2.0)
├── 頁面容器層 (1個Widget)
│   └── WelcomePage() - 主頁面容器及狀態管理
├── 佈局結構層 (1個Widget)
│   └── WelcomeScaffold() - 基礎頁面結構
├── 功能區塊層 (3個Widget)
│   ├── BrandSection() - 品牌展示區塊
│   ├── ModeSelectionSection() - 模式選擇區塊
│   └── NavigationSection() - 導航操作區塊
├── 視覺元件層 (2個Widget)
│   ├── BrandLogo() - 品牌Logo
│   └── BrandTitle() - 品牌標題文字
├── 佈局元件層 (1個Widget)
│   └── ModeGrid() - 模式卡片網格
├── 互動元件層 (2個Widget)
│   ├── ModeCard() - 模式選擇卡片
│   └── ActionButton() - 主要操作按鈕
└── 基礎元件層 (3個Widget)
    ├── GradientBackground() - 漸層背景
    ├── LoadingIndicator() - 載入指示器
    └── ErrorDialog() - 錯誤對話框
```

---

## 3.0 核心Widget設計

### 3.1 頁面容器層Widget

#### 3.1.1 WelcomePage Widget
```dart
/**
 * 01. 對應 3.1.1：主頁面容器Widget
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 歡迎頁面的主容器，支援首次啟動與再次啟動流程
 */
class WelcomePage extends StatefulWidget {
  static const String routeName = '/welcome';
  const WelcomePage({Key? key}) : super(key: key);
  @override
  State<WelcomePage> createState() => _WelcomePageState();
}

/**
 * 02. 對應 3.1.1：歡迎頁面狀態類別
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 管理歡迎頁面的狀態和動畫控制器
 */
class _WelcomePageState extends State<WelcomePage> with SingleTickerProviderStateMixin {
  late AnimationController _animationController;
  late Animation<double> _fadeAnimation;
  
  @override
  void initState();
  
  /**
   * 03. 對應 3.1.1：初始化動畫控制器
   * @version 2025-08-06-V2.2.0
   * @date 2025-08-06 15:00:00
   * @description 設定頁面淡入動畫效果
   */
  void _initializeAnimations();
  
  /**
   * 04. 對應 3.1.1：初始化頁面狀態
   * @version 2025-08-06-V2.2.0
   * @date 2025-08-06 15:00:00
   * @description 調用Provider進行頁面初始化
   */
  Future<void> _initializePage();
  
  @override
  Widget build(BuildContext context);
  
  @override
  void dispose();
}
```

### 3.2 佈局結構層Widget

#### 3.2.1 WelcomeScaffold Widget
```dart
/**
 * 05. 對應 3.2.1：頁面佈局結構Widget
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 提供歡迎頁面的基礎佈局結構，遵循9005文件響應式設計規範
 */
class WelcomeScaffold extends StatelessWidget {
  final Animation<double> fadeAnimation;
  final WelcomeProvider provider;
  
  const WelcomeScaffold({
    Key? key,
    required this.fadeAnimation,
    required this.provider,
  }) : super(key: key);

  @override
  Widget build(BuildContext context);
  
  /**
   * 06. 對應 3.2.1：建立錯誤覆蓋層
   * @version 2025-08-06-V2.2.0
   * @date 2025-08-06 15:00:00
   * @description 顯示錯誤對話框的覆蓋層
   */
  Widget _buildErrorOverlay(BuildContext context, WelcomeProvider provider);
}
```

### 3.3 功能區塊層Widget

#### 3.3.1 ModeSelectionSection Widget
```dart
/**
 * 03. 對應 3.3.1：模式選擇區塊Widget
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 四模式選擇介面，支援精準控制者/紀錄習慣者/轉型挑戰者/潛在覺醒者
 */
class ModeSelectionSection extends StatelessWidget {
  final WelcomeProvider provider;
  
  const ModeSelectionSection({
    Key? key,
    required this.provider,
  }) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        // 選擇提示文字
        Semantics(
          header: true,
          child: Text(
            '選擇適合您的使用模式',
            style: TextStyle(
              fontSize: 18.sp,
              color: Colors.white,
              fontWeight: FontWeight.w500,
            ),
            textAlign: TextAlign.center,
          ),
        ),
        
        SizedBox(height: 24.h),
        
        // 模式選擇網格
        Expanded(
          child: ModeGrid(
            selectedMode: provider.selectedMode,
            onModeSelected: provider.selectMode,
          ),
        ),
      ],
    );
  }
}
```

### 3.4 互動元件層Widget

#### 3.4.1 ModeCard Widget
```dart
/**
 * 04. 對應 3.4.1：模式選擇卡片Widget
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 單一模式選擇卡片，遵循0015文件四模式命名與9005色彩規範
 */
class ModeCard extends StatefulWidget {
  final UserMode mode;
  final bool isSelected;
  final VoidCallback onTap;
  
  const ModeCard({
    Key? key,
    required this.mode,
    required this.isSelected,
    required this.onTap,
  }) : super(key: key);

  @override
  State<ModeCard> createState() => _ModeCardState();
}

class _ModeCardState extends State<ModeCard> 
    with SingleTickerProviderStateMixin {
  
  late AnimationController _scaleController;
  late Animation<double> _scaleAnimation;
  
  @override
  void initState() {
    super.initState();
    _scaleController = AnimationController(
      duration: const Duration(milliseconds: 200),
      vsync: this,
    );
    _scaleAnimation = Tween<double>(
      begin: 1.0,
      end: 1.05,
    ).animate(CurvedAnimation(
      parent: _scaleController,
      curve: Curves.easeInOut,
    ));
  }

  @override
  Widget build(BuildContext context) {
    final modeConfig = _getModeConfig(widget.mode);
    final responsive = ResponsiveHelper.of(context);
    
    return AnimatedBuilder(
      animation: _scaleAnimation,
      builder: (context, child) {
        return Transform.scale(
          scale: widget.isSelected ? _scaleAnimation.value : 1.0,
          child: Semantics(
            button: true,
            enabled: true,
            label: modeConfig.semanticLabel,
            hint: '雙擊選擇${modeConfig.displayName}模式',
            child: InkWell(
              onTap: () {
                _scaleController.forward().then((_) {
                  _scaleController.reverse();
                });
                widget.onTap();
              },
              child: AnimatedContainer(
                duration: const Duration(milliseconds: 300),
                height: responsive.modeCardHeight,
                padding: EdgeInsets.all(16.w),
                decoration: BoxDecoration(
                  color: widget.isSelected 
                      ? modeConfig.selectedColor 
                      : modeConfig.normalColor,
                  borderRadius: BorderRadius.circular(16.r),
                  border: Border.all(
                    color: widget.isSelected 
                        ? Colors.white 
                        : Colors.transparent,
                    width: 2.w,
                  ),
                  boxShadow: widget.isSelected ? [
                    BoxShadow(
                      color: Colors.black26,
                      blurRadius: 8.r,
                      offset: Offset(0, 4.h),
                    ),
                  ] : null,
                ),
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    // 模式圖示
                    Icon(
                      modeConfig.icon,
                      size: responsive.isMobile ? 28.sp : 32.sp,
                      color: Colors.white,
                    ),
                    
                    SizedBox(height: 8.h),
                    
                    // 模式名稱
                    Text(
                      modeConfig.displayName,
                      style: TextStyle(
                        fontSize: responsive.isMobile ? 12.sp : 14.sp,
                        fontWeight: FontWeight.w600,
                        color: Colors.white,
                      ),
                      textAlign: TextAlign.center,
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                    ),
                    
                    SizedBox(height: 4.h),
                    
                    // 模式描述
                    Text(
                      modeConfig.shortDescription,
                      style: TextStyle(
                        fontSize: responsive.isMobile ? 9.sp : 10.sp,
                        color: Colors.white70,
                      ),
                      textAlign: TextAlign.center,
                      maxLines: 2,
                      overflow: TextOverflow.ellipsis,
                    ),
                  ],
                ),
              ),
            ),
          ),
        );
      },
    );
  }
  
  ModeConfig _getModeConfig(UserMode mode) {
    switch (mode) {
      case UserMode.controller:
        return ModeConfig(
          displayName: '精準控制者', // 0015規範命名
          shortDescription: '深度分析\n專業操作',
          semanticLabel: '精準控制者模式，適合需要深度分析和專業操作的使用者',
          icon: Icons.analytics,
          normalColor: const Color(0x331976D2), // 9005規範色彩
          selectedColor: const Color(0x661976D2),
        );
      case UserMode.logger:
        return ModeConfig(
          displayName: '紀錄習慣者', // 0015規範命名
          shortDescription: '優雅美觀\n習慣記錄',
          semanticLabel: '紀錄習慣者模式，重視優雅美觀體驗的記帳愛好者',
          icon: Icons.auto_awesome,
          normalColor: const Color(0x336A1B9A), // 9005規範色彩
          selectedColor: const Color(0x666A1B9A),
        );
      case UserMode.struggler:
        return ModeConfig(
          displayName: '轉型挑戰者', // 0015規範命名
          shortDescription: '目標導向\n習慣養成',
          semanticLabel: '轉型挑戰者模式，專注目標導向和習慣養成的使用者',
          icon: Icons.trending_up,
          normalColor: const Color(0x33FF6B35), // 9005規範色彩
          selectedColor: const Color(0x66FF6B35),
        );
      case UserMode.sleeper:
        return ModeConfig(
          displayName: '潛在覺醒者', // 0015規範命名
          shortDescription: '簡單易用\n輕鬆開始',
          semanticLabel: '潛在覺醒者模式，偏好簡單易用功能的入門使用者',
          icon: Icons.eco,
          normalColor: const Color(0x334CAF50), // 9005規範色彩
          selectedColor: const Color(0x664CAF50),
        );
    }
  }
  
  @override
  void dispose() {
    _scaleController.dispose();
    super.dispose();
  }
}
```

### 3.5 漸層背景Widget
```dart
/**
 * 05. 對應 3.5：漸層背景Widget
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 根據9005規範的四模式差異化背景
 */
class GradientBackground extends StatelessWidget {
  final UserMode? mode;
  
  const GradientBackground({Key? key, this.mode}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final gradient = _getGradientForMode(mode);
    
    return Container(
      decoration: BoxDecoration(
        gradient: gradient,
      ),
    );
  }
  
  LinearGradient _getGradientForMode(UserMode? mode) {
    switch (mode) {
      case UserMode.controller:
        return const LinearGradient(
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
          colors: [Color(0xFF1976D2), Color(0xFF37474F)], // 9005規範專業藍
        );
      case UserMode.logger:
        return const LinearGradient(
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
          colors: [Color(0xFF6A1B9A), Color(0xFFE1BEE7)], // 9005規範優雅紫
        );
      case UserMode.struggler:
        return const LinearGradient(
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
          colors: [Color(0xFFFF6B35), Color(0xFFFFE0B2)], // 9005規範活力橘
        );
      case UserMode.sleeper:
        return const LinearGradient(
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
          colors: [Color(0xFF4CAF50), Color(0xFFC8E6C9)], // 9005規範溫和綠
        );
      default:
        return const LinearGradient(
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
          colors: [Color(0xFF667eea), Color(0xFF764ba2)], // 預設漸層
        );
    }
  }
}
```

---

## 4.0 狀態管理架構

### 4.1 WelcomeProvider 狀態管理器
```dart
/**
 * 06. 對應 4.1：歡迎頁面狀態管理器
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 管理歡迎頁面的狀態，支援8701規範的完整功能流程
 */
class WelcomeProvider extends ChangeNotifier {
  // 頁面狀態 - 完整狀態枚舉定義（8701規範）
  WelcomePageState _pageState = WelcomePageState.loading;
  WelcomePageState get pageState => _pageState;
  
  // 使用者選擇的模式
  UserMode? _selectedMode;
  UserMode? get selectedMode => _selectedMode;
  
  // 載入狀態
  bool _isLoading = false;
  bool get isLoading => _isLoading;
  
  // 錯誤訊息與錯誤類型（8701規範）
  String? _errorMessage;
  String? get errorMessage => _errorMessage;
  
  WelcomeErrorType? _errorType;
  WelcomeErrorType? get errorType => _errorType;
  
  // 是否為首次啟動（8701規範）
  bool _isFirstLaunch = true;
  bool get isFirstLaunch => _isFirstLaunch;
  
  // 使用者認證狀態
  bool _isUserAuthenticated = false;
  bool get isUserAuthenticated => _isUserAuthenticated;
  
  // API服務
  final WelcomeApiService _apiService = WelcomeApiService();
  final NavigationService _navigationService = NavigationService();
  final PreferencesService _preferencesService = PreferencesService();

  /// 初始化頁面 - 支援首次啟動與再次啟動流程（8701規範）
  Future<void> initializePage() async {
    try {
      _setLoading(true);
      _setPageState(WelcomePageState.loading);
      
      // 檢查是否為首次啟動
      _isFirstLaunch = await _preferencesService.isFirstLaunch();
      
      if (!_isFirstLaunch) {
        // 再次啟動流程
        await _handleReturnUserFlow();
      } else {
        // 首次啟動流程
        await _handleFirstLaunchFlow();
      }
      
    } catch (e) {
      _setError('頁面初始化失敗', WelcomeErrorType.initialization, e);
      _setPageState(WelcomePageState.error);
    } finally {
      _setLoading(false);
    }
  }
  
  /// 處理首次啟動流程（8701規範）
  Future<void> _handleFirstLaunchFlow() async {
    // 檢查帳號刪除狀態 (對應F004)
    await _checkAccountDeletionStatus();
    
    // 顯示模式選擇
    _setPageState(WelcomePageState.modeSelection);
  }
  
  /// 處理再次啟動流程（8701規範）
  Future<void> _handleReturnUserFlow() async {
    // 檢查登入狀態
    _isUserAuthenticated = await _preferencesService.isLoggedIn();
    
    if (_isUserAuthenticated) {
      // 已登入用戶直接跳轉到首頁
      await _navigationService.navigateToHome();
    } else {
      // 未登入用戶跳轉到登入頁面
      await _navigationService.navigateToLogin();
    }
  }

  /// 檢查帳號刪除狀態（8701規範 - F004端點）
  Future<void> _checkAccountDeletionStatus() async {
    try {
      final response = await _apiService.checkInitialization();
      
      if (response.success && response.data != null) {
        final data = response.data!;
        
        if (data['accountDeletionRequired'] == true) {
          final deletionInfo = data['deletionPendingInfo'];
          _setError(
            '您的帳號將於 ${deletionInfo['scheduledDate']} 被刪除\n如需取消刪除，請聯繫客服',
            WelcomeErrorType.accountDeletion,
            null,
          );
          _setPageState(WelcomePageState.error);
        }
      }
    } catch (e) {
      // 檢查失敗不影響主流程，僅記錄錯誤
      debugPrint('帳號刪除狀態檢查失敗: $e');
    }
  }

  /// 選擇模式
  void selectMode(UserMode mode) {
    if (_selectedMode == mode) {
      _selectedMode = null;
    } else {
      _selectedMode = mode;
    }
    notifyListeners();
  }

  /// 開始使用 - 模式設定與導航（8701規範）
  Future<void> startApp() async {
    if (_selectedMode == null) {
      _setError('請先選擇一個使用模式', WelcomeErrorType.validation, null);
      return;
    }
    
    try {
      _setLoading(true);
      _setPageState(WelcomePageState.loading);
      
      // 調用模式設定API（修正為F009子功能）
      await _setUserMode(_selectedMode!);
      
      // 設定首次啟動標記
      await _preferencesService.setFirstLaunchCompleted();
      
      // 導航到登入頁面
      await _navigationService.navigateToLogin();
      
    } catch (e) {
      _setError('模式設定失敗', WelcomeErrorType.modeSelection, e);
      _setPageState(WelcomePageState.error);
    } finally {
      _setLoading(false);
    }
  }

  /// 設定使用者模式（修正為F009子功能）
  Future<void> _setUserMode(UserMode mode) async {
    final request = UserSettingsRequest(
      settingType: 'mode_selection',
      modeSettings: ModeSelectionData(
        selectedMode: mode.name,
        deviceInfo: await _getDeviceInfo(),
      ),
    );
    
    final response = await _apiService.updateUserSettings(request);
    
    if (!response.success) {
      throw Exception('模式設定失敗');
    }
    
    // 儲存模式配置到本地
    if (response.data?['modeConfig'] != null) {
      await _preferencesService.saveModeConfig(response.data!['modeConfig']);
    }
  }

  /// 取得裝置資訊
  Future<Map<String, dynamic>> _getDeviceInfo() async {
    return {
      'platform': Platform.isAndroid ? 'android' : 'ios',
      'version': await _getAppVersion(),
      'deviceId': await _getDeviceId(),
    };
  }

  /// 設定載入狀態
  void _setLoading(bool loading) {
    _isLoading = loading;
    notifyListeners();
  }

  /// 設定頁面狀態
  void _setPageState(WelcomePageState state) {
    _pageState = state;
    notifyListeners();
  }

  /// 設定錯誤訊息（8701規範 - 詳細錯誤處理）
  void _setError(String message, WelcomeErrorType type, dynamic error) {
    _errorMessage = message;
    _errorType = type;
    
    // 記錄詳細錯誤資訊
    if (error != null) {
      debugPrint('WelcomeProvider錯誤 [$type]: $message, 詳細: $error');
    }
    
    notifyListeners();
  }

  /// 清除錯誤
  void clearError() {
    _errorMessage = null;
    _errorType = null;
    
    // 根據錯誤類型決定後續動作
    if (_pageState == WelcomePageState.error) {
      if (_isFirstLaunch) {
        _setPageState(WelcomePageState.modeSelection);
      } else {
        // 重新初始化
        initializePage();
      }
    }
    
    notifyListeners();
  }
  
  /// 重試操作
  Future<void> retryOperation() async {
    switch (_errorType) {
      case WelcomeErrorType.initialization:
        await initializePage();
        break;
      case WelcomeErrorType.modeSelection:
        if (_selectedMode != null) {
          await startApp();
        }
        break;
      case WelcomeErrorType.networkConnection:
        await initializePage();
        break;
      default:
        clearError();
        break;
    }
  }
}
```

### 4.2 狀態枚舉定義（8701規範完整狀態）
```dart
/// 歡迎頁面狀態枚舉（8701規範）
enum WelcomePageState {
  loading,        // 載入中
  modeSelection,  // 模式選擇
  loginRedirect,  // 登入重導向
  error,          // 錯誤狀態
}

/// 使用者模式枚舉（0015規範命名）
enum UserMode {
  controller,  // 精準控制者
  logger,      // 紀錄習慣者
  struggler,   // 轉型挑戰者
  sleeper,     // 潛在覺醒者
}

/// 錯誤類型枚舉（8701規範詳細錯誤分類）
enum WelcomeErrorType {
  initialization,    // 初始化錯誤
  networkConnection, // 網路連線錯誤
  modeSelection,     // 模式選擇錯誤
  accountDeletion,   // 帳號刪除相關錯誤
  validation,        // 輸入驗證錯誤
  unknown,          // 未知錯誤
}
```

---

## 5.0 API整合設計

### 5.1 WelcomeApiService API服務（8701規範完整API整合）
```dart
/**
 * 07. 對應 5.1：歡迎頁面API服務
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 處理歡迎頁面相關的API調用，支援F004、F009端點（8701規範）
 */
class WelcomeApiService {
  final ApiClient _apiClient = ApiClient();

  /// 檢查應用程式初始化狀態（對應F004 - 8701規範）
  Future<ApiResponse<Map<String, dynamic>>> checkInitialization() async {
    try {
      final deviceInfo = await _getDeviceInfo();
      
      final response = await _apiClient.get(
        '/app/initialization',
        queryParameters: {
          'deviceInfo': deviceInfo,
          'accountDeletionCheck': true,
        },
      );
      
      return ApiResponse.success(response.data);
    } on DioError catch (e) {
      return ApiResponse.error(_handleDioError(e));
    } catch (e) {
      return ApiResponse.error('初始化檢查失敗: ${e.toString()}');
    }
  }

  /// 更新使用者設定（對應F009 - 修正為設定管理的子功能）
  Future<ApiResponse<Map<String, dynamic>>> updateUserSettings(
    UserSettingsRequest request,
  ) async {
    try {
      final response = await _apiClient.put(
        '/app/user/settings',
        data: request.toJson(),
      );
      
      if (response.data['success'] == true) {
        return ApiResponse.success(response.data['data']);
      } else {
        return ApiResponse.error(response.data['message'] ?? '設定更新失敗');
      }
    } on DioError catch (e) {
      return ApiResponse.error(_handleDioError(e));
    } catch (e) {
      return ApiResponse.error('使用者設定更新失敗: ${e.toString()}');
    }
  }

  /// 處理Dio錯誤（8701規範詳細錯誤處理）
  String _handleDioError(DioError error) {
    switch (error.type) {
      case DioErrorType.connectTimeout:
      case DioErrorType.receiveTimeout:
        return '網路連線逾時，請檢查網路設定';
      case DioErrorType.response:
        final statusCode = error.response?.statusCode;
        switch (statusCode) {
          case 400:
            return '請求參數錯誤，請重新操作';
          case 401:
            return '認證失效，請重新登入';
          case 403:
            return '存取權限不足';
          case 404:
            return '服務暫時無法使用';
          case 500:
            return '伺服器內部錯誤，請稍後重試';
          case 502:
            return '服務暫時無法回應，請稍後重試';
          case 503:
            return '服務維護中，請稍後重試';
          default:
            return '伺服器回應錯誤 (HTTP $statusCode)';
        }
      case DioErrorType.cancel:
        return '操作已取消';
      case DioErrorType.other:
        if (error.message.contains('SocketException')) {
          return '網路連線失敗，請檢查網路狀態';
        }
        return '網路錯誤，請稍後重試';
      default:
        return '未知網路錯誤';
    }
  }

  /// 取得裝置資訊
  Future<Map<String, dynamic>> _getDeviceInfo() async {
    final deviceInfo = DeviceInfoPlugin();
    
    if (Platform.isAndroid) {
      final androidInfo = await deviceInfo.androidInfo;
      return {
        'platform': 'android',
        'model': androidInfo.model,
        'version': androidInfo.version.release,
        'deviceId': androidInfo.androidId,
      };
    } else {
      final iosInfo = await deviceInfo.iosInfo;
      return {
        'platform': 'ios',
        'model': iosInfo.model,
        'version': iosInfo.systemVersion,
        'deviceId': iosInfo.identifierForVendor,
      };
    }
  }
}
```

### 5.2 資料模型定義（修正F009對應）
```dart
/// 使用者設定請求模型（修正為F009子功能）
class UserSettingsRequest {
  final String settingType;
  final ModeSelectionData? modeSettings;

  UserSettingsRequest({
    required this.settingType,
    this.modeSettings,
  });

  Map<String, dynamic> toJson() => {
    'settingType': settingType,
    'modeSettings': modeSettings?.toJson(),
  };
}

/// 模式選擇資料模型
class ModeSelectionData {
  final String selectedMode;
  final Map<String, dynamic> deviceInfo;

  ModeSelectionData({
    required this.selectedMode,
    required this.deviceInfo,
  });

  Map<String, dynamic> toJson() => {
    'selectedMode': selectedMode,
    'deviceInfo': deviceInfo,
  };
}

/// API回應統一格式
class ApiResponse<T> {
  final bool success;
  final T? data;
  final String? error;

  ApiResponse.success(this.data) : success = true, error = null;
  ApiResponse.error(this.error) : success = false, data = null;
}
```

---

## 6.0 導航路由設計

### 6.1 路由配置
```dart
/**
 * 08. 對應 6.1：歡迎頁面路由配置
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 定義歡迎頁面的路由規則與導航邏輯
 */
class WelcomeRoutes {
  static const String welcome = '/welcome';
  static const String login = '/login';
  static const String register = '/register';
  static const String home = '/home';

  static GoRoute getWelcomeRoute() {
    return GoRoute(
      path: welcome,
      name: 'welcome',
      builder: (context, state) => const WelcomePage(),
    );
  }
}

/// 導航服務
class NavigationService {
  static final GoRouter _router = GoRouter(
    initialLocation: '/welcome',
    routes: [
      WelcomeRoutes.getWelcomeRoute(),
      LoginRoutes.getLoginRoute(),
      RegisterRoutes.getRegisterRoute(),
      HomeRoutes.getHomeRoute(),
    ],
  );

  static GoRouter get router => _router;

  /// 導航到登入頁面
  Future<void> navigateToLogin() async {
    _router.pushReplacement('/login');
  }

  /// 導航到註冊頁面
  Future<void> navigateToRegister() async {
    _router.pushReplacement('/register');
  }

  /// 導航到首頁
  Future<void> navigateToHome() async {
    _router.pushReplacement('/home');
  }

  /// 返回上一頁
  void goBack() {
    _router.pop();
  }
}
```

---

## 7.0 響應式設計

### 7.1 響應式輔助工具（9005規範完整實作）
```dart
/**
 * 09. 對應 7.1：響應式設計輔助工具
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 遵循9005文件的響應式設計規範，提供完整的斷點管理
 */
class ResponsiveHelper {
  final BuildContext context;
  late final MediaQueryData _mediaQuery;
  late final Size _screenSize;

  ResponsiveHelper.of(BuildContext context) : context = context {
    _mediaQuery = MediaQuery.of(context);
    _screenSize = _mediaQuery.size;
  }

  /// 斷點判斷（9005規範）
  bool get isMobile => _screenSize.width < 480;
  bool get isTablet => _screenSize.width >= 481 && _screenSize.width < 768;
  bool get isSmallDesktop => _screenSize.width >= 769 && _screenSize.width < 1024;
  bool get isLargeDesktop => _screenSize.width >= 1025;

  /// 裝置類型
  DeviceType get deviceType {
    if (isMobile) return DeviceType.mobile;
    if (isTablet) return DeviceType.tablet;
    if (isSmallDesktop) return DeviceType.smallDesktop;
    return DeviceType.largeDesktop;
  }

  /// 水平內邊距（9005規範）
  double get horizontalPadding {
    switch (deviceType) {
      case DeviceType.mobile:
        return 16.w;
      case DeviceType.tablet:
        return 32.w;
      case DeviceType.smallDesktop:
        return 48.w;
      case DeviceType.largeDesktop:
        return 64.w;
    }
  }

  /// 垂直內邊距（9005規範）
  double get verticalPadding {
    switch (deviceType) {
      case DeviceType.mobile:
        return 24.h;
      case DeviceType.tablet:
        return 32.h;
      case DeviceType.smallDesktop:
        return 40.h;
      case DeviceType.largeDesktop:
        return 48.h;
    }
  }

  /// 模式網格列數（9005規範）
  int get modeGridCrossAxisCount {
    switch (deviceType) {
      case DeviceType.mobile:
        return 2; // 2x2 網格
      case DeviceType.tablet:
        return 4; // 4x1 網格
      case DeviceType.smallDesktop:
      case DeviceType.largeDesktop:
        return 4; // 4x1 網格
    }
  }

  /// 模式卡片高度（9005規範）
  double get modeCardHeight {
    switch (deviceType) {
      case DeviceType.mobile:
        return 120.h;
      case DeviceType.tablet:
        return 100.h;
      case DeviceType.smallDesktop:
        return 90.h;
      case DeviceType.largeDesktop:
        return 85.h;
    }
  }

  /// 按鈕高度（9005規範）
  double get buttonHeight {
    switch (deviceType) {
      case DeviceType.mobile:
        return 48.h;
      case DeviceType.tablet:
        return 52.h;
      case DeviceType.smallDesktop:
      case DeviceType.largeDesktop:
        return 48.h;
    }
  }

  /// 字型大小縮放（9005規範）
  double get fontSizeScale {
    switch (deviceType) {
      case DeviceType.mobile:
        return 1.0;
      case DeviceType.tablet:
        return 1.1;
      case DeviceType.smallDesktop:
        return 1.0;
      case DeviceType.largeDesktop:
        return 0.9;
    }
  }
}
```

### 7.2 斷點管理（9005規範）
```dart
/// 響應式斷點常數（9005規範）
class Breakpoints {
  static const double mobileMax = 480;      // 手機最大寬度
  static const double tabletMin = 481;      // 平板最小寬度
  static const double tabletMax = 768;      // 平板最大寬度
  static const double smallDesktopMin = 769; // 小型桌面最小寬度
  static const double smallDesktopMax = 1024; // 小型桌面最大寬度
  static const double largeDesktopMin = 1025; // 大型桌面最小寬度

  /// 取得目前裝置類型
  static DeviceType getDeviceType(double width) {
    if (width <= mobileMax) return DeviceType.mobile;
    if (width >= tabletMin && width <= tabletMax) return DeviceType.tablet;
    if (width >= smallDesktopMin && width <= smallDesktopMax) return DeviceType.smallDesktop;
    return DeviceType.largeDesktop;
  }
}

enum DeviceType { 
  mobile, 
  tablet, 
  smallDesktop, 
  largeDesktop 
}
```

---

## 8.0 無障礙功能（9005規範WCAG 2.1 AA標準）

### 8.1 語意化支援
```dart
/**
 * 10. 對應 8.1：無障礙語意化Widget包裝器
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 提供WCAG 2.1 AA標準的完整無障礙支援（9005規範）
 */
class AccessibilityWrapper {
  
  /// 為模式卡片提供語意化標籤
  static Widget wrapModeCard(Widget child, UserMode mode, bool isSelected) {
    final modeNames = {
      UserMode.controller: '精準控制者',
      UserMode.logger: '紀錄習慣者', 
      UserMode.struggler: '轉型挑戰者',
      UserMode.sleeper: '潛在覺醒者',
    };
    
    final modeDescriptions = {
      UserMode.controller: '適合需要深度分析和專業操作的使用者',
      UserMode.logger: '重視優雅美觀體驗的記帳愛好者',
      UserMode.struggler: '專注目標導向和習慣養成的使用者',
      UserMode.sleeper: '偏好簡單易用功能的入門使用者',
    };
    
    return Semantics(
      button: true,
      enabled: true,
      selected: isSelected,
      label: '${modeNames[mode]}模式',
      value: isSelected ? '已選擇' : '未選擇',
      hint: isSelected 
          ? '已選擇${modeNames[mode]}模式，${modeDescriptions[mode]}，雙擊取消選擇'
          : '選擇${modeNames[mode]}模式，${modeDescriptions[mode]}，雙擊確認選擇',
      onTapHint: '選擇此模式',
      child: child,
    );
  }

  /// 為品牌Logo提供語意化標籤
  static Widget wrapBrandLogo(Widget child) {
    return Semantics(
      image: true,
      label: 'LCAS 2.0 智慧記帳應用程式標誌',
      excludeSemantics: true,
      child: child,
    );
  }

  /// 為開始按鈕提供語意化標籤
  static Widget wrapStartButton(Widget child, {required bool enabled, UserMode? selectedMode}) {
    final modeNames = {
      UserMode.controller: '精準控制者',
      UserMode.logger: '紀錄習慣者', 
      UserMode.struggler: '轉型挑戰者',
      UserMode.sleeper: '潛在覺醒者',
    };
    
    return Semantics(
      button: true,
      enabled: enabled,
      label: '開始使用',
      value: enabled && selectedMode != null 
          ? '已選擇${modeNames[selectedMode]}模式' 
          : '尚未選擇模式',
      hint: enabled 
          ? '雙擊開始使用應用程式' 
          : '請先選擇一個使用模式',
      onTapHint: enabled ? '開始使用' : null,
      child: child,
    );
  }

  /// 為載入指示器提供語意化標籤
  static Widget wrapLoadingIndicator(Widget child, String? loadingMessage) {
    return Semantics(
      liveRegion: true,
      label: loadingMessage ?? '載入中，請稍候',
      child: child,
    );
  }

  /// 為錯誤對話框提供語意化標籤
  static Widget wrapErrorDialog(Widget child, String errorMessage, WelcomeErrorType errorType) {
    final errorTypeMessages = {
      WelcomeErrorType.networkConnection: '網路連線錯誤',
      WelcomeErrorType.modeSelection: '模式設定錯誤',
      WelcomeErrorType.initialization: '初始化錯誤',
      WelcomeErrorType.accountDeletion: '帳號狀態警告',
      WelcomeErrorType.validation: '輸入驗證錯誤',
      WelcomeErrorType.unknown: '系統錯誤',
    };
    
    return Semantics(
      liveRegion: true,
      assertiveness: Assertiveness.assertive,
      label: '錯誤訊息：${errorTypeMessages[errorType]}',
      value: errorMessage,
      hint: '按確定關閉錯誤訊息，或按重試重新操作',
      child: child,
    );
  }
}
```

### 8.2 色彩對比度支援（9005規範）
```dart
/// 高對比度主題支援（WCAG 2.1 AA標準）
class HighContrastTheme {
  
  /// 檢查是否啟用高對比度模式
  static bool isHighContrastEnabled(BuildContext context) {
    return MediaQuery.of(context).highContrast;
  }

  /// 檢查是否啟用大字型模式
  static bool isLargeTextEnabled(BuildContext context) {
    final textScaleFactor = MediaQuery.of(context).textScaleFactor;
    return textScaleFactor > 1.3;
  }

  /// 檢查是否啟用動畫減少模式
  static bool isReduceMotionEnabled(BuildContext context) {
    return MediaQuery.of(context).disableAnimations;
  }

  /// 取得高對比度文字色彩（對比度至少4.5:1）
  static Color getContrastTextColor(BuildContext context, Color backgroundColor) {
    if (isHighContrastEnabled(context)) {
      final luminance = backgroundColor.computeLuminance();
      return luminance > 0.5 ? Colors.black : Colors.white;
    }
    
    // 檢查一般對比度
    final luminance = backgroundColor.computeLuminance();
    if (luminance > 0.5) {
      return const Color(0xFF212121); // 深灰色，對白背景對比度16.31:1
    } else {
      return Colors.white; // 白色
    }
  }

  /// 取得高對比度按鈕色彩
  static Color getContrastButtonColor(BuildContext context, Color normalColor) {
    if (isHighContrastEnabled(context)) {
      return normalColor.computeLuminance() > 0.5 
          ? const Color(0xFF000080) // 深藍色
          : const Color(0xFFFFFFFF); // 白色
    }
    return normalColor;
  }

  /// 取得高對比度邊框色彩
  static Color getContrastBorderColor(BuildContext context) {
    if (isHighContrastEnabled(context)) {
      return Colors.black;
    }
    return Colors.transparent;
  }

  /// 取得適配的字型大小
  static double getScaledFontSize(BuildContext context, double baseFontSize) {
    final textScaleFactor = MediaQuery.of(context).textScaleFactor;
    
    // 限制最大縮放避免版面破壞
    final clampedScaleFactor = textScaleFactor.clamp(0.8, 2.0);
    
    return baseFontSize * clampedScaleFactor;
  }

  /// 取得適配的動畫時長
  static Duration getScaledAnimationDuration(BuildContext context, Duration baseDuration) {
    if (isReduceMotionEnabled(context)) {
      return const Duration(milliseconds: 1); // 幾乎無動畫
    }
    return baseDuration;
  }
}
```

### 8.3 觸控目標大小（WCAG 2.1 AA標準）
```dart
/// 觸控目標大小輔助工具
class TouchTargetHelper {
  // WCAG 2.1 AA標準最小觸控目標：44x44 dp
  static const double minTouchTargetSize = 44.0;
  
  /// 確保Widget符合最小觸控目標大小
  static Widget ensureMinTouchTarget(Widget child, {
    double? width,
    double? height,
  }) {
    return Container(
      constraints: BoxConstraints(
        minWidth: width ?? minTouchTargetSize,
        minHeight: height ?? minTouchTargetSize,
      ),
      child: child,
    );
  }

  /// 為小於標準的按鈕增加透明觸控區域
  static Widget expandTouchTarget(Widget child, {
    required double currentWidth,
    required double currentHeight,
  }) {
    final needsExpansion = currentWidth < minTouchTargetSize || 
                          currentHeight < minTouchTargetSize;
                          
    if (!needsExpansion) return child;
    
    final expandWidth = math.max(0, minTouchTargetSize - currentWidth) / 2;
    final expandHeight = math.max(0, minTouchTargetSize - currentHeight) / 2;
    
    return Padding(
      padding: EdgeInsets.symmetric(
        horizontal: expandWidth,
        vertical: expandHeight,
      ),
      child: child,
    );
  }
}
```

---

## 9.0 效能最佳化

### 9.1 Widget快取策略
```dart
/**
 * 11. 對應 9.1：Widget快取管理器
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 優化Widget重建效能，減少不必要的重建
 */
class WidgetCache {
  static final Map<String, Widget> _cache = {};
  static final Map<String, DateTime> _cacheTimestamps = {};
  static const Duration _cacheValidDuration = Duration(minutes: 5);

  /// 取得快取的Widget或建立新的
  static Widget getOrCreate(String key, Widget Function() creator) {
    final timestamp = _cacheTimestamps[key];
    final isExpired = timestamp == null || 
        DateTime.now().difference(timestamp) > _cacheValidDuration;

    if (_cache.containsKey(key) && !isExpired) {
      return _cache[key]!;
    }
    
    final widget = creator();
    _cache[key] = widget;
    _cacheTimestamps[key] = DateTime.now();
    return widget;
  }

  /// 清除過期快取
  static void clearExpiredCache() {
    final now = DateTime.now();
    final expiredKeys = <String>[];
    
    _cacheTimestamps.forEach((key, timestamp) {
      if (now.difference(timestamp) > _cacheValidDuration) {
        expiredKeys.add(key);
      }
    });
    
    for (final key in expiredKeys) {
      _cache.remove(key);
      _cacheTimestamps.remove(key);
    }
  }

  /// 清除所有快取
  static void clearAllCache() {
    _cache.clear();
    _cacheTimestamps.clear();
  }

  /// 取得快取統計資訊
  static Map<String, dynamic> getCacheStats() {
    return {
      'totalCached': _cache.length,
      'oldestCache': _cacheTimestamps.isEmpty 
          ? null 
          : _cacheTimestamps.values.reduce((a, b) => a.isBefore(b) ? a : b),
      'newestCache': _cacheTimestamps.isEmpty 
          ? null 
          : _cacheTimestamps.values.reduce((a, b) => a.isAfter(b) ? a : b),
    };
  }
}
```

### 9.2 圖片預載入與最佳化
```dart
/// 圖片預載入管理器
class ImagePreloader {
  static final Set<String> _preloadedImages = {};
  static final Map<String, Completer<void>> _loadingImages = {};

  /// 預載入品牌相關圖片資源
  static Future<void> preloadBrandAssets(BuildContext context) async {
    const assets = [
      'assets/images/logo.png',
      'assets/images/brand_background.png',
      'assets/icons/mode_controller.png',
      'assets/icons/mode_logger.png',
      'assets/icons/mode_struggler.png',
      'assets/icons/mode_sleeper.png',
    ];

    final preloadFutures = assets.map((asset) => _preloadSingleImage(context, asset));
    await Future.wait(preloadFutures);
  }

  /// 預載入單一圖片
  static Future<void> _preloadSingleImage(BuildContext context, String asset) async {
    if (_preloadedImages.contains(asset)) {
      return; // 已預載入
    }

    if (_loadingImages.containsKey(asset)) {
      return _loadingImages[asset]!.future; // 正在載入中
    }

    final completer = Completer<void>();
    _loadingImages[asset] = completer;

    try {
      await precacheImage(AssetImage(asset), context);
      _preloadedImages.add(asset);
      completer.complete();
    } catch (e) {
      debugPrint('圖片預載入失敗: $asset, 錯誤: $e');
      completer.completeError(e);
    } finally {
      _loadingImages.remove(asset);
    }
  }

  /// 檢查圖片是否已預載入
  static bool isImagePreloaded(String asset) {
    return _preloadedImages.contains(asset);
  }

  /// 清除預載入記錄
  static void clearPreloadedImages() {
    _preloadedImages.clear();
  }
}
```

### 9.3 記憶體管理與效能監控
```dart
/// 記憶體使用監控
class MemoryMonitor {
  static Timer? _monitorTimer;
  static final List<int> _memoryUsageHistory = [];
  static const int _maxHistoryLength = 60; // 保留60個監控點
  static const int _warningThresholdMB = 150; // 警告閾值150MB
  static const int _criticalThresholdMB = 200; // 危險閾值200MB

  /// 開始記憶體監控
  static void startMonitoring() {
    _monitorTimer?.cancel();
    
    _monitorTimer = Timer.periodic(const Duration(seconds: 30), (timer) {
      _checkMemoryUsage();
    });
  }

  /// 停止記憶體監控
  static void stopMonitoring() {
    _monitorTimer?.cancel();
    _monitorTimer = null;
  }

  /// 檢查記憶體使用狀況
  static void _checkMemoryUsage() {
    final usageMB = _getCurrentMemoryUsageMB();
    
    // 記錄歷史
    _memoryUsageHistory.add(usageMB);
    if (_memoryUsageHistory.length > _maxHistoryLength) {
      _memoryUsageHistory.removeAt(0);
    }

    // 檢查閾值
    if (usageMB > _criticalThresholdMB) {
      debugPrint('🔴 危險：記憶體使用量過高 ${usageMB}MB，執行緊急清理');
      _performEmergencyCleanup();
    } else if (usageMB > _warningThresholdMB) {
      debugPrint('🟡 警告：記憶體使用量較高 ${usageMB}MB，執行一般清理');
      _performRegularCleanup();
    }
  }

  /// 取得目前記憶體使用量（MB）
  static int _getCurrentMemoryUsageMB() {
    try {
      return (ProcessInfo.currentRss ~/ 1024 ~/ 1024);
    } catch (e) {
      return 0;
    }
  }

  /// 執行一般清理
  static void _performRegularCleanup() {
    WidgetCache.clearExpiredCache();
    // 其他輕量級清理操作
  }

  /// 執行緊急清理
  static void _performEmergencyCleanup() {
    WidgetCache.clearAllCache();
    ImagePreloader.clearPreloadedImages();
    
    // 強制垃圾回收（Android/iOS平台會自動處理，這裡主要用於提醒）
    if (Platform.isAndroid || Platform.isIOS) {
      // 移除不需要的引用
      _memoryUsageHistory.clear();
    }
  }

  /// 取得記憶體使用統計
  static Map<String, dynamic> getMemoryStats() {
    if (_memoryUsageHistory.isEmpty) return {};

    final currentUsage = _memoryUsageHistory.last;
    final averageUsage = _memoryUsageHistory.reduce((a, b) => a + b) / _memoryUsageHistory.length;
    final maxUsage = _memoryUsageHistory.reduce(math.max);
    final minUsage = _memoryUsageHistory.reduce(math.min);

    return {
      'current_mb': currentUsage,
      'average_mb': averageUsage.round(),
      'max_mb': maxUsage,
      'min_mb': minUsage,
      'samples': _memoryUsageHistory.length,
      'cache_stats': WidgetCache.getCacheStats(),
    };
  }
}
```

---

## 10.0 錯誤處理機制（8701規範詳細錯誤處理）

### 10.1 統一錯誤處理
```dart
/**
 * 12. 對應 10.1：歡迎頁面錯誤處理器
 * @version 2025-08-06-V2.2.0
 * @date 2025-08-06 15:00:00
 * @description 根據8701規範的具體錯誤情境進行詳細處理
 */
class WelcomeErrorHandler {
  
  /// 處理初始化錯誤（8701規範）
  static String handleInitializationError(dynamic error) {
    if (error is DioError) {
      return _handleNetworkError(error);
    }
    
    if (error.toString().contains('Firebase')) {
      return '資料庫連線失敗，請檢查網路連線';
    }
    
    if (error.toString().contains('Permission')) {
      return '應用程式權限不足，請重新安裝應用程式';
    }
    
    return '應用程式初始化失敗，請重新啟動應用程式';
  }

  /// 處理模式選擇錯誤（8701規範）
  static String handleModeSelectionError(dynamic error) {
    if (error is DioError) {
      switch (error.response?.statusCode) {
        case 400:
          return '模式設定參數錯誤，請重新選擇';
        case 409:
          return '模式設定衝突，請稍後重試';
        case 429:
          return '操作過於頻繁，請稍後重試';
        default:
          return _handleNetworkError(error);
      }
    }
    
    if (error.toString().contains('validation')) {
      return '請選擇一個有效的使用模式';
    }
    
    if (error.toString().contains('timeout')) {
      return '模式設定逾時，請檢查網路連線';
    }
    
    return '模式設定失敗，請稍後重試';
  }

  /// 處理網路錯誤（8701規範）
  static String _handleNetworkError(DioError error) {
    switch (error.type) {
      case DioErrorType.connectTimeout:
        return '連線逾時，請檢查網路設定';
      case DioErrorType.receiveTimeout:
        return '伺服器回應逾時，請稍後重試';
      case DioErrorType.sendTimeout:
        return '傳送資料逾時，請稍後重試';
      case DioErrorType.response:
        return _handleHttpStatusError(error.response?.statusCode);
      case DioErrorType.cancel:
        return '操作已取消';
      case DioErrorType.other:
        if (error.message.contains('SocketException')) {
          return '網路連線不穩定，請檢查網路狀態';
        }
        return '網路連線失敗，請稍後重試';
      default:
        return '未知網路錯誤';
    }
  }

  /// 處理HTTP狀態碼錯誤（8701規範）
  static String _handleHttpStatusError(int? statusCode) {
    switch (statusCode) {
      case 400:
        return '請求格式錯誤，請重試';
      case 401:
        return '認證失效，請重新登入';
      case 403:
        return '存取權限不足，請聯繫客服';
      case 404:
        return '服務暫時無法使用，請稍後重試';
      case 409:
        return '資料衝突，請重新操作';
      case 429:
        return '操作過於頻繁，請稍後重試';
      case 500:
        return '伺服器內部錯誤，請稍後重試';
      case 502:
        return '服務暫時無法回應，請稍後重試';
      case 503:
        return '服務維護中，請稍後重試';
      case 504:
        return '伺服器處理逾時，請稍後重試';
      default:
        return '伺服器錯誤 (HTTP $statusCode)，請稍後重試';
    }
  }

  /// 處理帳號刪除相關錯誤（8701規範）
  static String handleAccountDeletionError(Map<String, dynamic>? deletionInfo) {
    if (deletionInfo == null) {
      return '帳號狀態檢查失敗，請聯繫客服';
    }
    
    final scheduledDate = deletionInfo['scheduledDate'] ?? '未知';
    final reason = deletionInfo['reason'] ?? '使用者申請';
    
    return '您的帳號將於 $scheduledDate 被刪除\n刪除原因：$reason\n如需取消刪除，請立即聯繫客服';
  }

  /// 處理驗證錯誤（8701規範）
  static String handleValidationError(String field, dynamic value) {
    switch (field) {
      case 'mode':
        if (value == null) {
          return '請選擇一個使用模式';
        }
        return '選擇的模式無效，請重新選擇';
      case 'device_info':
        return '裝置資訊取得失敗，請重新啟動應用程式';
      case 'network_state':
        return '網路狀態異常，請檢查網路連線';
      default:
        return '輸入驗證失敗，請檢查輸入內容';
    }
  }

  /// 顯示錯誤對話框（8701規範）
  static Future<void> showErrorDialog(
    BuildContext context, 
    String message, 
    WelcomeErrorType errorType,
    {VoidCallback? onRetry, VoidCallback? onDismiss}
  ) async {
    final errorTitle = _getErrorTitle(errorType);
    final showRetryButton = _shouldShowRetryButton(errorType);
    
    return showDialog<void>(
      context: context,
      barrierDismissible: false,
      builder: (context) => AccessibilityWrapper.wrapErrorDialog(
        AlertDialog(
          title: Text(errorTitle),
          content: SingleChildScrollView(
            child: Text(
              message,
              style: TextStyle(
                fontSize: HighContrastTheme.getScaledFontSize(context, 14),
              ),
            ),
          ),
          actions: [
            if (showRetryButton && onRetry != null)
              TextButton(
                onPressed: () {
                  Navigator.of(context).pop();
                  onRetry();
                },
                child: const Text('重試'),
              ),
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
                onDismiss?.call();
              },
              child: const Text('確定'),
            ),
          ],
        ),
        message,
        errorType,
      ),
    );
  }

  /// 取得錯誤標題
  static String _getErrorTitle(WelcomeErrorType errorType) {
    switch (errorType) {
      case WelcomeErrorType.networkConnection:
        return '網路連線錯誤';
      case WelcomeErrorType.modeSelection:
        return '模式設定錯誤';
      case WelcomeErrorType.initialization:
        return '初始化錯誤';
      case WelcomeErrorType.accountDeletion:
        return '帳號狀態通知';
      case WelcomeErrorType.validation:
        return '輸入錯誤';
      case WelcomeErrorType.unknown:
      default:
        return '系統錯誤';
    }
  }

  /// 判斷是否應顯示重試按鈕
  static bool _shouldShowRetryButton(WelcomeErrorType errorType) {
    switch (errorType) {
      case WelcomeErrorType.networkConnection:
      case WelcomeErrorType.modeSelection:
      case WelcomeErrorType.initialization:
        return true;
      case WelcomeErrorType.accountDeletion:
      case WelcomeErrorType.validation:
      case WelcomeErrorType.unknown:
      default:
        return false;
    }
  }
}
```

### 10.2 錯誤復原機制（8701規範）
```dart
/// 錯誤復原策略
class ErrorRecovery {
  
  /// 自動重試機制（指數退避）
  static Future<T> retryWithBackoff<T>(
    Future<T> Function() operation, {
    int maxRetries = 3,
    Duration initialDelay = const Duration(seconds: 1),
    double backoffMultiplier = 2.0,
    Duration maxDelay = const Duration(seconds: 30),
  }) async {
    Exception? lastException;
    
    for (int attempt = 0; attempt < maxRetries; attempt++) {
      try {
        return await operation();
      } catch (e) {
        lastException = e is Exception ? e : Exception(e.toString());
        
        if (attempt < maxRetries - 1) {
          final delay = _calculateDelay(
            initialDelay, 
            attempt, 
            backoffMultiplier, 
            maxDelay
          );
          
          debugPrint('重試 ${attempt + 1}/$maxRetries 失敗，${delay.inSeconds}秒後重試: $e');
          await Future.delayed(delay);
        }
      }
    }
    
    throw lastException!;
  }

  /// 計算退避延遲時間
  static Duration _calculateDelay(
    Duration initialDelay,
    int attempt,
    double multiplier,
    Duration maxDelay,
  ) {
    final delayMs = (initialDelay.inMilliseconds * math.pow(multiplier, attempt)).round();
    return Duration(milliseconds: math.min(delayMs, maxDelay.inMilliseconds));
  }

  /// 智慧重試（根據錯誤類型決定是否重試）
  static Future<T> smartRetry<T>(
    Future<T> Function() operation,
    bool Function(dynamic error) shouldRetry, {
    int maxRetries = 3,
  }) async {
    Exception? lastException;
    
    for (int attempt = 0; attempt < maxRetries; attempt++) {
      try {
        return await operation();
      } catch (e) {
        lastException = e is Exception ? e : Exception(e.toString());
        
        if (attempt < maxRetries - 1 && shouldRetry(e)) {
          final delay = Duration(seconds: (attempt + 1) * 2);
          debugPrint('智慧重試 ${attempt + 1}/$maxRetries，延遲${delay.inSeconds}秒: $e');
          await Future.delayed(delay);
        } else {
          break; // 不應重試或已達最大重試次數
        }
      }
    }
    
    throw lastException!;
  }

  /// 判斷是否應該重試
  static bool shouldRetryForError(dynamic error) {
    if (error is DioError) {
      switch (error.type) {
        case DioErrorType.connectTimeout:
        case DioErrorType.receiveTimeout:
        case DioErrorType.sendTimeout:
          return true; // 網路逾時應重試
        case DioErrorType.response:
          final statusCode = error.response?.statusCode;
          return statusCode != null && (
            statusCode >= 500 || // 伺服器錯誤
            statusCode == 408 || // 請求逾時
            statusCode == 429    // 請求過於頻繁
          );
        case DioErrorType.other:
          return error.message.contains('SocketException'); // 網路連線問題
        default:
          return false;
      }
    }
    
    // 其他類型錯誤的重試判斷
    final errorStr = error.toString().toLowerCase();
    return errorStr.contains('timeout') || 
           errorStr.contains('connection') ||
           errorStr.contains('network');
  }

  /// 離線模式處理
  static Widget buildOfflineWidget({
    VoidCallback? onRetry,
    String? message,
  }) {
    return Container(
      padding: const EdgeInsets.all(24),
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          const Icon(
            Icons.wifi_off,
            size: 64,
            color: Colors.grey,
          ),
          const SizedBox(height: 24),
          Text(
            message ?? '目前處於離線狀態',
            style: const TextStyle(
              fontSize: 18, 
              color: Colors.grey,
              fontWeight: FontWeight.w500,
            ),
            textAlign: TextAlign.center,
          ),
          const SizedBox(height: 12),
          const Text(
            '請檢查網路連線後重試',
            style: TextStyle(
              fontSize: 14,
              color: Colors.grey,
            ),
            textAlign: TextAlign.center,
          ),
          if (onRetry != null) ...[
            const SizedBox(height: 24),
            ElevatedButton.icon(
              onPressed: onRetry,
              icon: const Icon(Icons.refresh),
              label: const Text('重試'),
              style: ElevatedButton.styleFrom(
                padding: const EdgeInsets.symmetric(
                  horizontal: 32,
                  vertical: 16,
                ),
              ),
            ),
          ],
        ],
      ),
    );
  }

  /// 錯誤邊界Widget
  static Widget buildErrorBoundary({
    required Widget child,
    Widget Function(String error)? errorBuilder,
  }) {
    return Builder(
      builder: (context) {
        try {
          return child;
        } catch (e) {
          if (errorBuilder != null) {
            return errorBuilder(e.toString());
          }
          
          return Container(
            padding: const EdgeInsets.all(16),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                const Icon(
                  Icons.error_outline,
                  size: 48,
                  color: Colors.red,
                ),
                const SizedBox(height: 16),
                const Text(
                  '介面載入失敗',
                  style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.w500,
                  ),
                ),
                const SizedBox(height: 8),
                Text(
                  e.toString(),
                  style: const TextStyle(fontSize: 12),
                  textAlign: TextAlign.center,
                ),
              ],
            ),
          );
        }
      },
    );
  }
}
```

---

## 文件維護記錄

| 版本 | 日期 | 修改者 | 修改內容 |
|------|------|--------|----------|
| **v2.2.0** | **2025-08-06 15:00:00 +08:00** | **LCAS SA Team** | **專注技術設計實作：1.移除測試案例相關內容，專注於技術實作設計 2.統一代碼節點編號格式，遵循0055文件規範 3.升級所有Widget、模組、函數版本至v2.2.0 4.保留完整的Widget架構、狀態管理、API整合、響應式設計、無障礙支援、效能最佳化及錯誤處理機制** |
| v2.1.0 | 2025-08-06 14:30:00 +08:00 | LCAS SA Team | 修正與規範文件違背：1.統一採用0015規範的四模式中文命名（精準控制者/紀錄習慣者/轉型挑戰者/潛在覺醒者）2.修正API端點對應，將模式選擇改為F009使用者設定管理的子功能 3.完整實作9005規範的色彩系統、響應式設計、UI元件規格、WCAG 2.1 AA無障礙標準 4.補全8701規範的首次啟動/再次啟動流程、完整狀態管理、F004+F009 API整合、詳細錯誤處理機制 |
| v2.0.0 | 2025-08-06 12:00:00 +08:00 | LCAS SA Team | 重大版本升級：完全重構為技術設計文件，參照5005文件格式，包含完整的Widget架構設計、狀態管理、API整合、響應式設計、無障礙支援、效能最佳化及錯誤處理機制 |

---

## 備註

- **專注技術實作**：移除測試案例相關內容，專注於技術設計與實作細節
- **規範對齊完成**：解決與0015、9005、8701文件的所有違背問題
- **代碼節點標準化**：遵循0055規範，統一代碼節點編號格式
- **四模式命名統一**：採用0015規範的中文命名系統
- **API整合修正**：模式選擇功能正確對應F009使用者設定管理
- **完整響應式實作**：遵循9005規範的斷點、佈局、字型縮放系統
- **WCAG 2.1 AA標準**：完整無障礙功能實作，包含語意化、對比度、觸控目標
- **8701功能流程完整實作**：支援首次/再次啟動、完整狀態管理、詳細錯誤處理
- **效能最佳化**：Widget快取、圖片預載、記憶體監控
- **錯誤處理機制**：根據8701規範實作詳細的錯誤分類與處理策略

