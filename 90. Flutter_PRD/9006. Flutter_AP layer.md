...
├── 📝 基礎記帳群組 (F006-F009)
│   ├── F006: 新增記帳記錄 → EntryService.createEntry()
│   ├── F007: 快速記帳 → EntryService.quickEntry()
│   ├── F008: 查詢記帳歷史 → EntryService.getEntries()
│   └── F009: 記帳記錄管理 → EntryService.updateEntry() / deleteEntry()
├── 📊 多帳本管理群組 (F010-F015)
│   ├── F010-F011: 專案管理 → ProjectLedgerService.createProject()
│   ├── F012-F013: 帳本切換 → ProjectLedgerService.switchLedger()
│   └── F014-F015: 成員管理 → ProjectLedgerService.manageMembers()
├── 💰 預算管理群組 (F016-F018)
│   ├── F016: 預算設定 → BudgetService.createBudget()
│   ├── F017: 預算進度 → BudgetService.getBudgetProgress()
│   └── F018: 預算提醒 → BudgetService.setBudgetAlerts()
├── 🤝 協作功能群組 (F019-F021)
│   ├── F019: 共享帳本 → CollaborationService.createSharedLedger()
│   ├── F020: 權限管理 → CollaborationService.setMemberPermissions()
│   └── F021: 即時同步 → CollaborationService.realtimeSync()
├── 📈 報表功能群組 (F022-F024)
│   ├── F022: 報表生成 → ReportService.generateReport()
│   ├── F023: 自訂報表 → ReportService.createCustomReport()
│   └── F024: 報表匯出 → ReportService.exportReport()
└── 🔧 系統管理群組 (F025-F032)
    ├── F025-F027: 備份管理 → SystemService.scheduleBackup()
    ├── F028-F029: 同步管理 → SystemService.checkSyncStatus()
    ├── F030: 系統健康 → SystemService.getSystemHealth()
    ├── F031: 錯誤日誌 → SystemService.getErrorLogs()
    └── F032: 提醒功能 → SystemService.createReminder()
```
...
```
F006 新增記帳記錄 ──對應──► entry_remote_datasource.dart.createEntry()
F007 快速記帳     ──對應──► entry_remote_datasource.dart.quickEntry()
F008 查詢記帳歷史 ──對應──► entry_remote_datasource.dart.getEntries()
F009 記帳記錄管理 ──對應──► entry_remote_datasource.dart.updateEntry()
                           └── entry_remote_datasource.dart.deleteEntry()
```
