...
2. [系統層級架構](#20-系統層級架構system-level-architecture)
3. [AP Layer模組架構](#30-ap-layer模組架構ap-layer-module-architecture)
4. [程式碼組織架構](#40-程式碼組織架構code-organization-architecture)
5. [架構關係對應](#50-架構關係對應architecture-relationship-mapping)
6. [API服務層設計](#60-api服務層設計api-services-layer)
7. [32個API端點整合](#70-32個api端點整合api-endpoints-integration)
8. [資料模型定義](#80-資料模型定義data-models)
9. [狀態管理策略](#90-狀態管理策略state-management)
10. [錯誤處理機制](#100-錯誤處理機制error-handling)
11. [快取策略設計](#110-快取策略設計caching-strategy)
12. [離線同步機制](#120-離線同步機制offline-sync)
13. [效能優化策略](#130-效能優化策略performance-optimization)
14. [測試策略](#140-測試策略testing-strategy)
15. [版本升級計畫](#150-版本升級計畫version-upgrade)
...
```
LCAS 2.0 系統架構
├── Flutter Presentation Layer (展示層)
│   ├── UI Components → 使用者介面元件
│   ├── Screens & Views → 畫面與視圖
│   └── User Interactions → 使用者互動
├── Flutter AP Layer (應用邏輯層) ★ 本文件範圍 ★
│   ├── 32個API端點 → F001-F032
│   ├── State Management → 狀態管理
│   ├── Data Transformation → 資料轉換
│   └── Cache Management → 快取管理
├── Business Layer (商業邏輯層)
│   ├── API Integration → API整合服務
│   └── Business Logic → 商業邏輯
└── Data Layer (資料層)
    ├── Collections → 資料集合
    ├── Documents → 文件資料
...
## 2.0 系統層級架構（System Level Architecture）

### 2.1 LCAS 2.0系統架構概覽

Flutter AP Layer在整個LCAS 2.0系統中扮演**應用邏輯層**的角色，負責連接前端展示層與後端商業邏輯層：

```
┌─────────────────────────────────────────┐
│        Flutter Presentation Layer       │  ← 9005文件範圍
│    (UI Components, Screens, Views)      │
└─────────────────┬───────────────────────┘
                  │ Widget Events & State Updates
┌─────────────────▼───────────────────────┐
│         Flutter AP Layer ★              │  ← 9006文件範圍
│  ┌─────────────────────────────────────┐ │
│  │       9大服務群組 (F001-F032)         │ │
│  │ AuthService | EntryService | ...    │ │
│  └─────────────────────────────────────┘ │
│  ┌─────────────────────────────────────┐ │
│  │   Repository & Use Cases Pattern    │ │
│  └─────────────────────────────────────┘ │
│  ┌─────────────────────────────────────┐ │
...
└─────────────────┬───────────────────────┘
                  │ HTTP/HTTPS Requests
┌─────────────────▼───────────────────────┐
│         RESTful API Layer               │  ← 6001文件範圍
│         (32個API端點)                   │
└─────────────────┬───────────────────────┘
                  │ Database Operations
┌─────────────────▼───────────────────────┐
│          Firestore Database             │  ← 6002文件範圍
...
#### 2.2.1 向上資料流（往Presentation Layer）
- **狀態更新**: Provider/Riverpod狀態變更
- **事件回調**: 操作結果通知
- **錯誤處理**: 使用者友善的錯誤訊息

#### 2.2.2 向下資料流（往Business Layer）
- **API請求**: RESTful HTTP請求
- **資料驗證**: 請求參數驗證
- **認證管理**: JWT Token自動附加

#### 2.2.3 橫向資料流（AP Layer內部）
- **服務協作**: 9大服務群組間的協調
- **快取管理**: 本地資料快取與同步
- **狀態同步**: 全域狀態與區域狀態同步
...
## 3.0 AP Layer模組架構（AP Layer Module Architecture）

### 3.1 功能性分層架構

AP Layer內部採用功能性分層，包含9大服務群組對應32個API端點：

```
Flutter AP Layer 功能性架構
├── 🔐 認證與帳戶管理服務 (F001-F005)
│   ├── F001: 使用者註冊 → AuthService.registerUser()
│   ├── F002: 使用者登入 → AuthService.loginUser()
│   ├── F003: 使用者登出 → AuthService.logoutUser()
│   ├── F004: 帳號刪除 → AuthService.deleteAccount()
│   └── F005: 密碼重設 → AuthService.resetPassword()
├── 💰 基礎記帳服務 (F006-F009)
│   ├── F006: APP記帳功能 → EntryService.createEntry()
│   ├── F007: APP記錄查詢 → EntryService.queryEntries()
│   ├── F008: 科目代碼管理 → EntryService.getSubjectsList()
│   └── F009: 使用者設定管理 → EntryService.updateUserSettings()
├── 📖 多帳本管理服務 (F010-F015)
│   ├── F010: 專案帳本建立 → ProjectLedgerService.createProject()
│   ├── F011: 專案帳本管理 → ProjectLedgerService.manageProject()
│   ├── F012: 專案帳本刪除 → ProjectLedgerService.deleteProject()
│   ├── F013: 分類帳本建立 → ProjectLedgerService.createCategory()
│   ├── F014: 分類帳本管理 → ProjectLedgerService.manageCategory()
│   └── F015: 多帳本切換 → ProjectLedgerService.switchLedger()
├── 💵 預算管理服務 (F016-F018)
│   ├── F016: 預算設定建立 → BudgetService.createBudget()
│   ├── F017: 預算追蹤監控 → BudgetService.monitorBudget()
│   └── F018: 預算警示設定 → BudgetService.setBudgetAlerts()
├── 👥 協作服務 (F019-F021)
│   ├── F019: 共享帳本建立 → CollaborationService.createSharedLedger()
│   ├── F020: 多人協作權限 → CollaborationService.setMemberPermissions()
│   └── F021: 即時協作同步 → CollaborationService.realtimeSync()
├── 📈 報表服務 (F022-F024)
│   ├── F022: 標準報表產出 → ReportService.generateReport()
│   ├── F023: 自定義報表設計 → ReportService.createCustomReport()
│   └── F024: 報表匯出功能 → ReportService.exportReport()
├── 💾 備份與同步服務 (F025-F028)
│   ├── F025: 定期自動備份 → SystemService.scheduleBackup()
│   ├── F026: 手動備份還原 → SystemService.manualBackup()
│   ├── F027: 備份檔案管理 → SystemService.manageBackup()
│   └── F028: 資料同步檢查 → SystemService.checkSyncStatus()
├── 🖥️ 系統監控服務 (F029-F030)
│   ├── F029: 系統健康監控 → SystemService.getSystemHealth()
│   └── F030: 錯誤日誌管理 → SystemService.getErrorLogs()
└── ⏰ 排程提醒服務 (F031-F032)
    ├── F031: 排程提醒設定 → SystemService.createReminder()
    └── F032: 排程提醒執行 → SystemService.executeReminder()
```

### 3.2 服務群組職責矩陣

| 服務群組 | 主要職責 | API端點數量 | 核心功能 | 相依性 |
|----------|----------|-------------|----------|--------|
| **AuthService** | 認證與帳戶服務 | 5個 (F001-F005) | 登入註冊、權限驗證、帳戶管理 | 獨立服務 |
| **EntryService** | 基礎記帳服務 | 4個 (F006-F009) | CRUD記帳記錄、科目管理 | 依賴AuthService |
| **ProjectLedgerService** | 多帳本管理服務 | 6個 (F010-F015) | 專案帳本、分類管理、帳本切換 | 依賴AuthService |
| **BudgetService** | 預算管理服務 | 3個 (F016-F018) | 預算設定、進度追蹤、警示管理 | 依賴EntryService |
| **CollaborationService** | 協作服務 | 3個 (F019-F021) | 共享帳本、權限管理、即時同步 | 依賴ProjectLedgerService |
| **ReportService** | 報表服務 | 3個 (F022-F024) | 標準報表、自定義報表、匯出功能 | 依賴EntryService |
| **BackupSyncService** | 備份與同步服務 | 4個 (F025-F028) | 自動備份、手動備份、同步檢查 | 依賴所有服務 |
| **MonitoringService** | 系統監控服務 | 2個 (F029-F030) | 健康監控、錯誤日誌管理 | 獨立服務 |
| **ScheduleService** | 排程提醒服務 | 2個 (F031-F032) | 提醒設定、排程執行 | 依賴AuthService |

---

## 4.0 程式碼組織架構（Code Organization Architecture）

### 4.1 Clean Architecture實作結構

AP Layer採用Clean Architecture模式，確保程式碼的可維護性和可測試性：

```dart
lib/  ← Flutter AP Layer 程式碼根目錄
├── core/  ← 🏗️ 基礎設施層 (Infrastructure Layer)
│   ├── api/
│   │   ├── api_client.dart          → 統一API客戶端
│   │   ├── api_interceptors.dart    → 請求攔截器
│   │   └── api_endpoints.dart       → API端點定義 (F001-F032)
│   ├── cache/
...
│   └── di/
│       └── service_locator.dart     → 依賴注入設定
├── data/  ← 📊 資料層 (Data Layer)
│   ├── datasources/
│   │   ├── remote/  ← 🌐 9大服務群組的遠端資料源
│   │   │   ├── auth_remote_datasource.dart           → F001-F005
│   │   │   ├── entry_remote_datasource.dart          → F006-F009
│   │   │   ├── project_ledger_remote_datasource.dart → F010-F015
│   │   │   ├── budget_remote_datasource.dart         → F016-F018
│   │   │   ├── collaboration_remote_datasource.dart  → F019-F021
│   │   │   ├── report_remote_datasource.dart         → F022-F024
│   │   │   ├── backup_sync_remote_datasource.dart    → F025-F028
│   │   │   ├── monitoring_remote_datasource.dart     → F029-F030
│   │   │   └── schedule_remote_datasource.dart       → F031-F032
│   │   └── local/  ← 💾 本地快取資料源
│   │       ├── auth_local_datasource.dart
│   │       ├── entry_local_datasource.dart
│   │       ├── project_ledger_local_datasource.dart
│   │       ├── budget_local_datasource.dart
│   │       ├── collaboration_local_datasource.dart
│   │       ├── report_local_datasource.dart
│   │       ├── backup_sync_local_datasource.dart
│   │       ├── monitoring_local_datasource.dart
...
│       ├── project_ledger_repository_impl.dart
│       ├── budget_repository_impl.dart
│       ├── collaboration_repository_impl.dart
...
│   │   ├── project.dart
│   │   └── ...
│   ├── repositories/  ← 📜 Repository抽象介面
│   │   ├── auth_repository.dart
│   │   ├── entry_repository.dart
│   │   ├── project_ledger_repository.dart
...
│   └── usecases/  ← ⚙️ 業務用例 (Use Cases)
│       ├── auth/
│       │   ├── login_user.dart      → F002 登入用例
│       │   ├── register_user.dart   → F001 註冊用例
│       │   └── ...
│       ├── entry/
│       │   ├── create_entry.dart    → F006 新增記帳用例
│       │   ├── get_entries.dart     → F007 查詢記帳用例
│       │   └── ...
│       ├── project_ledger/
│       │   ├── create_project.dart  → F010 建立專案用例
│       │   ├── switch_ledger.dart   → F015 切換帳本用例
│       │   └── ...
│       ├── budget/
│       │   ├── create_budget.dart   → F016 建立預算用例
│       │   └── ...
│       ├── collaboration/
│       │   ├── create_shared_ledger.dart → F019 建立共享帳本用例
│       │   └── ...
│       ├── report/
│       │   ├── generate_report.dart → F022 產生報表用例
│       │   └── ...
│       ├── backup_sync/
│       │   ├── schedule_backup.dart → F025 排程備份用例
│       │   └── ...
│       ├── monitoring/
│       │   ├── check_system_health.dart → F029 系統健康檢查用例
│       │   └── ...
│       └── schedule/
│           ├── create_reminder.dart → F031 建立提醒用例
│           └── ...
└── presentation/  ← 🎨 展示層接口 (與9005文件的銜接點)
    ├── providers/  ← 🔄 狀態管理
    │   ├── auth_provider.dart            → 認證狀態管理
    │   ├── entry_provider.dart           → 記帳狀態管理
    │   ├── project_ledger_provider.dart  → 多帳本狀態管理
    │   ├── budget_provider.dart          → 預算狀態管理
    │   ├── collaboration_provider.dart   → 協作狀態管理
    │   ├── report_provider.dart          → 報表狀態管理
    │   ├── backup_sync_provider.dart     → 備份同步狀態管理
    │   ├── monitoring_provider.dart      → 系統監控狀態管理
    │   └── schedule_provider.dart        → 排程提醒狀態管理
    └── widgets/  ← 🧩 共用UI元件 (由9005文件詳細定義)
        ├── common/
        └── ...
```
...
Clean Architecture的依賴規則在AP Layer中的實作：

```
🎨 Presentation ──依賴──► 🎯 Domain ◄──實作── 📊 Data
     │                      ▲                    │
     └──────依賴──────────────┘                    │
                                                 ▼
                                            🏗️ Core
...
**依賴原則**：
- **Presentation Layer**: 依賴Domain的Use Cases和Entities
- **Data Layer**: 實作Domain的Repository介面
- **Domain Layer**: 完全獨立，不依賴任何外部層
- **Core Layer**: 被所有層共用，提供基礎設施

---

## 5.0 架構關係對應（Architecture Relationship Mapping）

### 5.1 三層架構對應關係

以下表格說明**系統層級架構**、**模組功能架構**、**程式碼組織架構**之間的對應關係：

| 系統層級架構 | 模組功能架構 | 程式碼組織架構 | 說明 |
|-------------|-------------|---------------|------|
| **Flutter AP Layer** | **9大服務群組** | **lib/** | 整個AP Layer的範圍 |
| ↳ Business Logic | ↳ AuthService (F001-F005) | ↳ data/datasources/remote/auth_remote_datasource.dart | 認證服務實作 |
| ↳ Business Logic | ↳ EntryService (F006-F009) | ↳ data/datasources/remote/entry_remote_datasource.dart | 記帳服務實作 |
| ↳ Business Logic | ↳ ProjectLedgerService (F010-F015) | ↳ data/datasources/remote/project_ledger_remote_datasource.dart | 多帳本服務實作 |
| ↳ Business Logic | ↳ BudgetService (F016-F018) | ↳ data/datasources/remote/budget_remote_datasource.dart | 預算服務實作 |
| ↳ Business Logic | ↳ CollaborationService (F019-F021) | ↳ data/datasources/remote/collaboration_remote_datasource.dart | 協作服務實作 |
| ↳ Business Logic | ↳ ReportService (F022-F024) | ↳ data/datasources/remote/report_remote_datasource.dart | 報表服務實作 |
| ↳ Business Logic | ↳ BackupSyncService (F025-F028) | ↳ data/datasources/remote/backup_sync_remote_datasource.dart | 備份同步服務實作 |
| ↳ Business Logic | ↳ MonitoringService (F029-F030) | ↳ data/datasources/remote/monitoring_remote_datasource.dart | 系統監控服務實作 |
| ↳ Business Logic | ↳ ScheduleService (F031-F032) | ↳ data/datasources/remote/schedule_remote_datasource.dart | 排程提醒服務實作 |
| ↳ API Integration | ↳ HTTP Client | ↳ core/api/api_client.dart | 統一API客戶端 |
| ↳ State Management | ↳ Repository Pattern | ↳ domain/repositories/ + data/repositories/ | 資料存取抽象 |
| ↳ Data Transformation | ↳ Data Models | ↳ data/models/ + domain/entities/ | 資料模型轉換 |
| ↳ Cache Management | ↳ Cache Strategies | ↳ core/cache/ + data/datasources/local/ | 快取管理 |

### 5.2 API端點到程式碼的對應

#### 5.2.1 認證管理群組 (F001-F005) 對應
```
F001 使用者註冊 ──對應──► auth_remote_datasource.dart.registerUser()
                   ├── domain/usecases/auth/register_user.dart
                   ├── data/repositories/auth_repository_impl.dart
                   └── presentation/providers/auth_provider.dart

F002 使用者登入 ──對應──► auth_remote_datasource.dart.loginUser()
F003 使用者登出 ──對應──► auth_remote_datasource.dart.logoutUser()
F004 帳號刪除   ──對應──► auth_remote_datasource.dart.deleteAccount()
F005 密碼重設   ──對應──► auth_remote_datasource.dart.resetPassword()
```

#### 5.2.2 基礎記帳群組 (F006-F009) 對應
```
F006 APP記帳功能     ──對應──► entry_remote_datasource.dart.createEntry()
F007 APP記錄查詢     ──對應──► entry_remote_datasource.dart.queryEntries()
F008 科目代碼管理    ──對應──► entry_remote_datasource.dart.getSubjectsList()
F009 使用者設定管理  ──對應──► entry_remote_datasource.dart.updateUserSettings()
```

#### 5.2.3 多帳本管理群組 (F010-F015) 對應
```
F010 專案帳本建立 ──對應──► project_ledger_remote_datasource.dart.createProject()
                   ├── domain/usecases/project_ledger/create_project.dart
                   ├── data/repositories/project_ledger_repository_impl.dart
                   └── presentation/providers/project_ledger_provider.dart

F011 專案帳本管理 ──對應──► project_ledger_remote_datasource.dart.manageProject()
F012 專案帳本刪除 ──對應──► project_ledger_remote_datasource.dart.deleteProject()
F013 分類帳本建立 ──對應──► project_ledger_remote_datasource.dart.createCategory()
F014 分類帳本管理 ──對應──► project_ledger_remote_datasource.dart.manageCategory()
F015 多帳本切換   ──對應──► project_ledger_remote_datasource.dart.switchLedger()
```

#### 5.2.4 預算管理群組 (F016-F018) 對應
```
F016 預算設定建立 ──對應──► budget_remote_datasource.dart.createBudget()
                   ├── domain/usecases/budget/create_budget.dart
                   ├── data/repositories/budget_repository_impl.dart
                   └── presentation/providers/budget_provider.dart

F017 預算追蹤監控 ──對應──► budget_remote_datasource.dart.monitorBudget()
F018 預算警示設定 ──對應──► budget_remote_datasource.dart.setBudgetAlerts()
```

#### 5.2.5 協作功能群組 (F019-F021) 對應
```
F019 共享帳本建立 ──對應──► collaboration_remote_datasource.dart.createSharedLedger()
                   ├── domain/usecases/collaboration/create_shared_ledger.dart
                   ├── data/repositories/collaboration_repository_impl.dart
                   └── presentation/providers/collaboration_provider.dart

F020 多人協作權限 ──對應──► collaboration_remote_datasource.dart.setMemberPermissions()
F021 即時協作同步 ──對應──► collaboration_remote_datasource.dart.realtimeSync()
```

#### 5.2.6 報表功能群組 (F022-F024) 對應
```
F022 標準報表產出 ──對應──► report_remote_datasource.dart.generateReport()
                   ├── domain/usecases/report/generate_report.dart
                   ├── data/repositories/report_repository_impl.dart
                   └── presentation/providers/report_provider.dart

F023 自定義報表設計 ──對應──► report_remote_datasource.dart.createCustomReport()
F024 報表匯出功能   ──對應──► report_remote_datasource.dart.exportReport()
```

#### 5.2.7 備份與同步群組 (F025-F028) 對應
```
F025 定期自動備份 ──對應──► backup_sync_remote_datasource.dart.scheduleBackup()
                   ├── domain/usecases/backup_sync/schedule_backup.dart
                   ├── data/repositories/backup_sync_repository_impl.dart
                   └── presentation/providers/backup_sync_provider.dart

F026 手動備份還原 ──對應──► backup_sync_remote_datasource.dart.manualBackup()
F027 備份檔案管理 ──對應──► backup_sync_remote_datasource.dart.manageBackup()
F028 資料同步檢查 ──對應──► backup_sync_remote_datasource.dart.checkSyncStatus()
```

#### 5.2.8 系統監控群組 (F029-F030) 對應
```
F029 系統健康監控 ──對應──► monitoring_remote_datasource.dart.getSystemHealth()
                   ├── domain/usecases/monitoring/check_system_health.dart
                   ├── data/repositories/monitoring_repository_impl.dart
                   └── presentation/providers/monitoring_provider.dart

F030 錯誤日誌管理 ──對應──► monitoring_remote_datasource.dart.getErrorLogs()
```

#### 5.2.9 排程提醒群組 (F031-F032) 對應
```
F031 排程提醒設定 ──對應──► schedule_remote_datasource.dart.createReminder()
                   ├── domain/usecases/schedule/create_reminder.dart
                   ├── data/repositories/schedule_repository_impl.dart
                   └── presentation/providers/schedule_provider.dart

F032 排程提醒執行 ──對應──► schedule_remote_datasource.dart.executeReminder()
```

### 5.3 9大服務群組在Clean Architecture中的定位

回答使用者的核心問題：**9大服務群組在分層架構設計的哪裡？**

```
Clean Architecture分層架構
│
├── 🎯 Domain Layer (領域層)
│   ├── entities/ → 業務實體定義
│   ├── repositories/ → Repository抽象介面
│   └── usecases/ → 業務用例實作 ★ 9大服務群組的業務邏輯在這裡 ★
│       ├── auth/ → AuthService的Use Cases
│       ├── entry/ → EntryService的Use Cases
│       ├── project_ledger/ → ProjectLedgerService的Use Cases
│       ├── budget/ → BudgetService的Use Cases
│       ├── collaboration/ → CollaborationService的Use Cases
│       ├── report/ → ReportService的Use Cases
│       ├── backup_sync/ → BackupSyncService的Use Cases
│       ├── monitoring/ → MonitoringService的Use Cases
│       └── schedule/ → ScheduleService的Use Cases
│
├── 📊 Data Layer (資料層)
│   └── datasources/remote/ ★ 9大服務群組的API調用在這裡 ★
│       ├── auth_remote_datasource.dart → AuthService API實作
│       ├── entry_remote_datasource.dart → EntryService API實作
│       ├── project_ledger_remote_datasource.dart → ProjectLedgerService API實作
│       ├── budget_remote_datasource.dart → BudgetService API實作
│       ├── collaboration_remote_datasource.dart → CollaborationService API實作
│       ├── report_remote_datasource.dart → ReportService API實作
│       ├── backup_sync_remote_datasource.dart → BackupSyncService API實作
│       ├── monitoring_remote_datasource.dart → MonitoringService API實作
│       └── schedule_remote_datasource.dart → ScheduleService API實作
│
└── 🎨 Presentation Layer (展示接口層)
    └── providers/ ★ 9大服務群組的狀態管理在這裡 ★
        ├── auth_provider.dart → AuthService狀態管理
        ├── entry_provider.dart → EntryService狀態管理
        ├── project_ledger_provider.dart → ProjectLedgerService狀態管理
        ├── budget_provider.dart → BudgetService狀態管理
        ├── collaboration_provider.dart → CollaborationService狀態管理
        ├── report_provider.dart → ReportService狀態管理
        ├── backup_sync_provider.dart → BackupSyncService狀態管理
        ├── monitoring_provider.dart → MonitoringService狀態管理
        └── schedule_provider.dart → ScheduleService狀態管理
```

**總結回答**：
1. **9大服務群組的業務邏輯** → `domain/usecases/` 目錄
2. **9大服務群組的API調用** → `data/datasources/remote/` 目錄  
3. **9大服務群組的狀態管理** → `presentation/providers/` 目錄
...
## 6.0 API服務層設計（API Services Layer）
...
## 7.0 32個API端點整合（API Endpoints Integration）
...
class ErrorInterceptor extends Interceptor {
  ...
  Future<void> _handleTokenExpired() async {
    ...
    // Assume ToastService.showError('登入已過期，請重新登入') is available
...
## 13.0 效能優化策略（Performance Optimization）
  ...
  group('AuthRepository', () {
    group('registerUser', () {
      ...
      test('should return ServerFailure when remote call throws ServerException', () async {
        ...
        when(mockNetworkInfo.isConnected).thenAnswer((_) async => true);
        when(mockRemoteDataSource.registerUser(any)).thenThrow(const ServerException(message: '伺服器錯誤'));
  ...
  group('API Integration Tests', () {
    ...
    group('Entry Management Flow', () {
      ...
      test('complete entry CRUD flow should work', () async {
        ...
        expect(foundEntry.description, '午餐');
...
void main() {
  ...
  group('EntryProvider', () {
    ...
    test('should handle create entry failure and set error message', () async {
      ...
      expect(entryProvider.isLoading, false); // Loading should be false
      expect(entryProvider.error, '建立失敗'); // Error message should be set
    });
...
## 15.0 版本升級計畫（Version Upgrade）

### 15.1 模組版本規劃

| 模組名稱 | 當前版本 | 目標版本 | 升級內容 | 相依性 |
|----------|----------|----------|----------|--------|
| **Flutter_AP_Layer** | v2.0.0 | v2.1.0 | 架構關係明確化，功能無變更 | 所有相關模組 |
| **AuthService** | - | v2.1.0 | F001-F005 API服務實作 | ApiClient v2.1.0 |
| **EntryService** | - | v2.1.0 | F006-F009 API服務實作 | ApiClient v2.1.0 |
| **ProjectLedgerService** | - | v2.1.0 | F010-F015 API服務實作 | ApiClient v2.1.0 |
| **BudgetService** | - | v2.1.0 | F016-F018 API服務實作 | ApiClient v2.1.0 |
| **CollaborationService** | - | v2.1.0 | F019-F021 API服務實作 | ApiClient v2.1.0 |
| **ReportService** | - | v2.1.0 | F022-F024 API服務實作 | ApiClient v2.1.0 |
| **SystemService** | - | v2.1.0 | F025-F032 API服務實作 | ApiClient v2.1.0 |
...
| API端點 | 函數名稱 | 版本 | 升級內容 |
|---------|----------|------|----------|
| F001-F005 | AuthService各方法 | v2.1.0 | 完整認證流程實作，架構優化 |
| F006-F009 | EntryService各方法 | v2.1.0 | 記帳CRUD操作實作，架構優化 |
| F010-F015 | ProjectLedgerService各方法 | v2.1.0 | 多帳本管理實作，架構優化 |
| F016-F018 | BudgetService各方法 | v2.1.0 | 預算管理實作，架構優化 |
| F019-F021 | CollaborationService各方法 | v2.1.0 | 協作功能實作，架構優化 |
| F022-F024 | ReportService各方法 | v2.1.0 | 報表功能實作，架構優化 |
| F025-F032 | SystemService各方法 | v2.1.0 | 系統管理實作，架構優化 |
...
| **v2.1.0** | **2025-01-26** | **LCAS SA Team** | **架構重組優化：分離系統、模組、程式碼三層架構描述，新增架構關係對應章節，明確7大服務群組在Clean Architecture中的位置，解決架構層次混淆問題** |

---

## 備註

- **版本2.1.0重大改進**: 解決架構層次混淆問題，明確系統、模組、程式碼三層架構關係
- **架構關係釐清**: 新增第5章專門說明7大服務群組在Clean Architecture中的具體位置
- **完整API整合**: 32個RESTful API端點(F001-F032)完整對應到程式碼實作
...
- **架構問題解決**: 回答「7大服務群組在分層架構設計的哪裡？」→ 答案在第5.3節