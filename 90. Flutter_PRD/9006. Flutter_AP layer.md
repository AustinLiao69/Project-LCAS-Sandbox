...
AP Layer內部採用功能性分層，包含7大服務群組對應32個API端點：
...
|----------|----------|-------------|----------|--------|
| **AuthService** | 認證與帳戶管理 | 5個 (F001-F005) | 登入註冊、權限驗證、帳戶管理 | 獨立服務 |
| **EntryService** | 基礎記帳功能 | 4個 (F006-F009) | CRUD記帳記錄、科目管理 | 依賴AuthService |
| **ProjectLedgerService** | 多帳本管理功能 | 6個 (F010-F015) | 專案帳本、分類管理、帳本切換 | 依賴AuthService |
| **BudgetService** | 預算管理功能 | 3個 (F016-F018) | 預算設定、進度追蹤、警示管理 | 依賴EntryService |
| **CollaborationService** | 協作功能 | 3個 (F019-F021) | 共享帳本、權限管理、即時同步 | 依賴ProjectLedgerService |
| **ReportService** | 報表功能 | 3個 (F022-F024) | 標準報表、自定義報表、匯出功能 | 依賴EntryService |
| **BackupSyncService** | 備份與同步功能 | 4個 (F025-F028) | 自動備份、手動備份、同步檢查 | 依賴所有服務 |
| **MonitoringService** | 系統監控功能 | 2個 (F029-F030) | 健康監控、錯誤日誌管理 | 獨立服務 |
| **ScheduleService** | 排程提醒功能 | 2個 (F031-F032) | 提醒設定、排程執行 | 依賴AuthService |
...
│   │   │   └── system_remote_datasource.dart        → F025-F032
│   │   └── local/  ← 💾 本地快取資料源
...
|-------------|-------------|---------------|------|
| **Flutter AP Layer** | **7大服務群組** | **lib/** | 整個AP Layer的範圍 |
...
| ↳ Business Logic | ↳ SystemService (F025-F032) | ↳ data/datasources/remote/system_remote_datasource.dart | 系統服務實作 |
...
### 5.3 7大服務群組在Clean Architecture中的定位

回答使用者的核心問題：**7大服務群組在分層架構設計的哪裡？**
...
│   ├── entities/ → 業務實體定義
│   ├── repositories/ → Repository抽象介面
│   └── usecases/ → 業務用例實作 ★ 7大服務群組的業務邏輯在這裡 ★
...
│       └── system/ → SystemService的Use Cases
│
├── 📊 Data Layer (資料層)
│   └── datasources/remote/ ★ 7大服務群組的API調用在這裡 ★
...
│       └── system_remote_datasource.dart → SystemService API實作
│
└── 🎨 Presentation Layer (展示接口層)
    └── providers/ ★ 7大服務群組的狀態管理在這裡 ★
...
**總結回答**：
1. **7大服務群組的業務邏輯** → `domain/usecases/` 目錄
2. **7大服務群組的API調用** → `data/datasources/remote/` 目錄  
3. **7大服務群組的狀態管理** → `presentation/providers/` 目錄
