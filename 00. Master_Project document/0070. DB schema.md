...
*| userSettings | object | optional | 用戶設定 | see below |
...
**Subcollections**:
| Subcollection | Description | Owner |
|---------------|-------------|-------------|
| wallets | 帳戶列表 | WCM模組 |
| categories | 科目列表  | WCM模組 |
| budgets | 預算設定 | BM模組 |
| transactions | 交易記錄 | LBK、BK模組 |
| pendingTransactions | 暫存記帳資料  | LBK、BK模組 |
...
| Field | Type | Required | Description | Constraints |
|-------|------|----------|-------------|-------------|
| UID | string | required | 擁有者ID | must exist in users |
| ledgerStatus | string | required | 狀態 | enum: active/archived 
...
### 4.2.1 Subcollection: ledgers/{ledgerId}/wallets
**Document ID**: cash/debit/credit
**Subcollection Path**: `ledgers/{ledgerId}/wallets`
**管理模組**: WCM (帳戶與科目管理)
**用途**: 帳戶(錢包)
**配置來源**: `0302. Default_wallet.json`

#### 4.2.1.1 Document: ledgers/{ledgerId}/wallets/cash
**Fields**:
| Field | Type | Required | Description | Constraints |
|-------|------|----------|-------------|-------------|
| UID | string | required | 用戶ID | must exist in users |
| ledgerId | string | required | 帳本ID | |
| walletId | string | required | 帳戶ID | primary key |
| walletName | string | optional | 帳戶名稱 | max 50 chars |
| walletBalance | number | required | 帳戶餘額 | |
| walletSynonyms | string | optional | 同義詞 | |
| walletStatus | string | required | 狀態 | enum: active/inactive |
| createdAt | timestamp | required | 建立時間 | |
| updatedAt | timestamp | required | 更新時間 | |
...
#### 4.2.1.2 Document: ledgers/{ledgerId}/wallets/debit
**Fields**:
| Field | Type | Required | Description | Constraints |
|-------|------|----------|-------------|-------------|
| UID | string | required | 用戶ID | must exist in users |
...
### 4.2.1.3 Document: ledgers/{ledgerId}/wallets/credit
**Fields**:
| Field | Type | Required | Description | Constraints |
|-------|------|----------|-------------|-------------|
| UID | string | required | 用戶ID | must exist in users |
...
### 4.2.2 (待改)Subcollection: ledgers/{ledgerId}/categories
**Document ID**: 101/102/103/104/105/106/107/108/201/301/999
...
#### 4.2.3 (待改)Subcollection: ledgers/{ledgerId}/pendingTransactions
**Document ID**: 隨機生成
...
#### 4.2.4 (待改)Subcollection: ledgers/{ledgerId}/budgets
**Document ID**: TBD
...
#### 4.2.5 (待新增)Subcollection: ledgers/{ledgerId}/transactions
**Document ID**: TBD
...
#### 4.3.1.3 Subcollection: collaborations/{ledgerId}/wallets
...
#### 4.3.1.4 Subcollection: collaborations/{ledgerId}/categories
...
#### 4.3.1.5 Subcollection: collaborations/{ledgerId}/pendingTransactions
...
#### 4.3.1.6 Subcollection: collaborations/{ledgerId}/budgets
...
#### 4.3.1.7 Subcollection: collaborations/{ledgerId}/transactions
