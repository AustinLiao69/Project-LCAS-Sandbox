...
### 1.3 版本資訊
**當前模組版本**:
- **AM模組**: v8.0.0（移除科目相關函數，專注帳號管理和帳本基礎結構）
- **BM模組**: v2.3.0（完全禁用頂層budgets集合，強制使用子集合架構）
- **CM模組**: v2.4.0（整合MLS功能，成為協作與帳本管理模組）
- **WCM模組**: v1.2.0（從AM模組接管科目初始化，支援動態路徑解析）
...
| managedBy | string | optional | 管理模組標識 | default: CM_v2.4.0 |
| subcollections | object | optional | 子集合架構 | 包含members, invitations, permissions, transactions, wallets, budgets, categories等完整架構 |
