...
| Field | Type | Required | Description | Constraints |
|-------|------|----------|-------------|-------------|
| walletId | string | optional | 帳戶ID | primary key |
...
```javascript
walletSettings: {
  walletInitialamount: number,     // 帳起始金額
  currency:string                  // 貨幣單位(預設；NTD)
}
```

#### 4.2.1.2 (待改)Subcollection: ledgers/{ledgerId}/categories
...
#### 4.2.1.3 (待改)Subcollection: ledgers/{ledgerId}/pendingTransactions
**Document ID**: 隨機生成
...
#### 4.2.1.4 (待改)Subcollection: ledgers/{ledgerId}/budgets
**Document ID**: TBD
...
#### 4.2.1.5 (待新增)Subcollection: ledgers/{ledgerId}/transactions
**Document ID**: TBD
