...
### 1.1 設計目的與範圍
記帳交易服務API Gateway提供LCAS 2.0系統記帳交易功能的統一API端點，負責接收HTTP請求並轉發至BL層處理。本文件定義純API Gateway的設計規範，專注於RESTful API端點定義、統一回應格式、錯誤處理與BL層轉發機制。
...
根據8103 API規格文件，記帳交易服務提供以下RESTful端點：

| HTTP方法 | 端點路徑 | 功能描述 | BL層函數 |
|---------|----------|----------|----------|
| POST | `/transactions/quick` | 快速記帳 | `1315.LBK.processQuickEntry()` |
| GET | `/transactions` | 查詢交易列表 | `1301.BK.getTransactionList()` |
| POST | `/transactions` | 建立交易記錄 | `1301.BK.createTransaction()` |
| GET | `/transactions/{id}` | 取得交易詳情 | `1301.BK.getTransaction()` |
| PUT | `/transactions/{id}` | 更新交易記錄 | `1301.BK.updateTransaction()` |
| DELETE | `/transactions/{id}` | 刪除交易記錄 | `1301.BK.deleteTransaction()` |
| GET | `/transactions/dashboard` | 儀表板數據 | `1341.MRA.getDashboardData()` |
| GET | `/transactions/statistics` | 統計分析 | `1341.MRA.getStatistics()` |
| POST | `/transactions/batch` | 批次操作 | `1301.BK.batchProcess()` |

### 3.2 快速記帳端點規範

**POST `/transactions/quick`**
```javascript
// 端點職責：接收快速記帳請求並轉發至BL層
// BL層函數：1315.LBK.processQuickEntry(requestData)
// 回應格式：遵循8088統一回應規範
...
```javascript
// 轉發至：1301.BK.getTransactionList(queryParams)
Query Parameters:
- page: 頁碼 (預設: 1)
- limit: 每頁筆數 (預設: 20)
- startDate: 開始日期
...
```javascript
// 轉發至：1301.BK.createTransaction(transactionData)
Request Body:
{
  "amount": 1500,
  "type": "expense",
...
```javascript
// 轉發至：1301.BK.getTransaction(transactionId)
Path Parameters:
- id: 交易記錄ID
```

**PUT `/transactions/{id}`** - 更新交易記錄
```javascript
// 轉發至：1301.BK.updateTransaction(transactionId, updateData)
```

**DELETE `/transactions/{id}`** - 刪除交易記錄
```javascript
// 轉發至：1301.BK.deleteTransaction(transactionId, options)
```
...
| `TRANSACTION_NOT_FOUND` | 404 | 交易記錄不存在 |
| `DUPLICATE_TRANSACTION` | 409 | 重複的交易記錄 |
...
| API端點 | BL層模組 | 函數名稱 |
|---------|----------|----------|
| `POST /transactions/quick` | 1315.LBK.js | `processQuickEntry()` |
| `GET /transactions` | 1301.BK.js | `getTransactionList()` |
| `POST /transactions` | 1301.BK.js | `createTransaction()` |
| `GET /transactions/{id}` | 1301.BK.js | `getTransaction()` |
| `PUT /transactions/{id}` | 1301.BK.js | `updateTransaction()` |
| `DELETE /transactions/{id}` | 1301.BK.js | `deleteTransaction()` |
| `GET /transactions/dashboard` | 1341.MRA.js | `getDashboardData()` |
| `GET /transactions/statistics` | 1341.MRA.js | `getStatistics()` |
...
```javascript
function adaptResponseForUserMode(data, userMode) {
  switch(userMode) {
    case 'Expert':
      return includeAdvancedFeatures(data);
    case 'Cultivation':
      return addGamificationElements(data);
    case 'Guiding':
