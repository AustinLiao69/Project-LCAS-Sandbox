...
### 4. [API端點設計](#3-api端點設計)
### 5. [統一回應格式](#4-統一回應格式)
### 6. [API 設計規格](#5-api-設計規格)
### 7. [資料模型設計](#6-資料模型設計)
### 8. [安全與驗證設計](#7-安全與驗證設計)
### 9. [錯誤處理設計](#8-錯誤處理設計)
### 10. [四模式支援設計](#9-四模式支援設計)
### 11. [測試設計策略](#10-測試設計策略)
...
```javascript
{
  ...
  "error": {
    ...
    "field": "backupType",
    "details": {
      "backupId": "backup_20250919_001",
      "reason": "儲存空間不足",
      "requiredSpace": "500MB",
      "availableSpace": "100MB",
      "suggestions": ["清理舊備份", "升級儲存空間"]
    }
  },
  "message": "備份操作失敗",
  "metadata": {
...
#### 6.1.2 BackupResponseFilter 類別規格
...
#### 6.2.1 Expert Mode 回應特性
- 包含詳細的備份統計和分析資料
- 提供進階備份選項和自訂設定
- 顯示完整的技術資訊和元數據

#### 6.2.2 Guiding Mode 回應特性
- 極簡化的備份狀態展示
- 隱藏複雜的技術選項
- 提供一鍵備份和還原功能
