...
└── statistics/          // 統計分析頁面
    ...
    └── statistics_service.dart          // 統計API客戶端（消費8001 /transactions/charts API）
```

**API消費說明**：
- **transaction_service.dart**: 消費8001的 `/transactions/*` 系列API（20個端點）
- **selection_service.dart**: 消費8001的 `/categories/*`, `/accounts/*`, `/ledgers/*` API
- **statistics_service.dart**: 消費8001的 `/transactions/charts`, `/transactions/statistics` API
...
abstract class BaseApiService {
  ...
  // 所有service都使用8001定義的132個API端點
  // 不定義新的API，只是封裝HTTP呼叫邏輯
}

// 範例：交易API服務
class TransactionService extends BaseApiService {
  Future<List<Transaction>> getTransactions() async {
    return await apiClient.get('/transactions'); // 8001定義的端點
  }
  
  Future<Transaction> createTransaction(TransactionData data) async {
    return await apiClient.post('/transactions', data); // 8001定義的端點
  }
}
```
