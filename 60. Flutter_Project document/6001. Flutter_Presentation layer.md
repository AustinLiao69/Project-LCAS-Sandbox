...
### 1.4 技術定位
PL 採用 Flutter 跨平台框架，基於 Dart 語言開發，支援：
...
基於6021 Screen list的40個畫面，將Presentation Layer分為5大模組群，每個模組群對應特定的功能範圍與畫面群組：

### 4.1 進入階段模組群（Entry Module Group）
...
### 4.2 記帳核心模組群（Bookkeeping Core Module Group）
...
### 4.4 分析智慧模組群（Analytics Intelligence Module Group）
...
```dart
// 分析智慧模組群架構
analytics_intelligence_module_group/
├── report_analytics/     // 報表分析模組
...
void publishEvent(String eventType, dynamic data) {
  AppStateManagement.eventBus.fire(ModuleEvent(
...
void subscribeToEvents() {
  AppStateManagement.eventBus.on<ModuleEvent>().listen((event) {
    if (event.targetModule == 'analytics') {
...
class ModeCoordinator {
  ...
  static List<Widget> filterFeaturesByMode(List<Widget> features, UserMode mode) {
    return features.where((feature) {
      return feature.isAvailableForMode(mode);
    }).toList();
  }
}