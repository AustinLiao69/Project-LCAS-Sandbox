...
│   ├── APL.dart統一Gateway集成 (Unified API Gateway)
...
本功能群採用三層架構設計：
- **消費API服務**：透過APL.dart統一Gateway進行API轉發，APL.dart提供完整的帳本管理API端點。
...
**重要原則**：**透過APL.dart統一Gateway**進行API轉發，APL.dart提供完整的帳本管理API端點。

#### 4.1.1 帳本管理API客戶端（透過APL.dart）
