...
本功能群採用三層架構設計：
- **消費API服務**：透過APL.dart統一Gateway進行API轉發，該Gateway提供14個帳本管理API端點。81資料夾已廢棄，所有API轉發統一透過APL.dart處理。
...
**重要原則**：**透過APL.dart統一Gateway**進行API轉發，使用APL.dart提供的14個帳本管理API端點。81資料夾已廢棄，所有API轉發統一透過APL.dart處理。
