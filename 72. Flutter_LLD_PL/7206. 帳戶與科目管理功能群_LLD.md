 ...
 * @update: 初版建立
 */
Future<ApiResponse<List<AccountModel>>> getWalletListWithPagination(String ledgerId, int page, int pageSize);
```
 ...
 * @update: 初版建立
 */
Future<ApiResponse<List<AccountModel>>> batchUpdateWalletStatus(List<String> walletIds, bool isActive);
```
...
| 序號 | 函數名稱 | 分類 | 用途說明 |
|------|----------|------|----------|
| 01 | getAccountList | 帳戶管理 | 取得帳戶清單 |
| 02 | createAccount | 帳戶管理 | 建立新帳戶 |
| 03 | updateAccount | 帳戶管理 | 更新帳戶資訊 |
| 04 | deleteAccount | 帳戶管理 | 刪除帳戶 |
| 05 | getCategoryTree | 科目管理 | 取得科目樹狀結構 |
| 06 | createCategory | 科目管理 | 建立新科目 |
| 07 | updateCategory | 科目管理 | 更新科目資訊 |
| 08 | deleteCategory | 科目管理 | 刪除科目 |
| 09 | getAccountBalance | 餘額管理 | 取得帳戶餘額 |
| 10 | getMultipleAccountBalances | 餘額管理 | 取得多帳戶餘額 |
| 11 | calculateLedgerTotalBalance | 餘額管理 | 計算帳本總餘額 |
| 12 | refreshAccountState | 狀態管理 | 重新整理帳戶狀態 |
| 13 | refreshCategoryState | 狀態管理 | 重新整理科目狀態 |
| 14 | syncDataState | 狀態管理 | 同步資料狀態 |
| 15 | validateAccountName | 工具函數 | 驗證帳戶名稱 |
...
2. getAccountList() → 取得現有帳戶
3. createAccount() / updateAccount() → 建立或更新帳戶
4. refreshAccountState() → 更新狀態
5. getAccountBalance() → 取得最新餘額
