...
void main() {
  group('用戶管理服務測試套件 v2.4.0 - 完整49個測試案例', () {
    ...
    /// 模擬帶性能特性的API調用
    Future<Map<String, dynamic>> _mockApiCall(String endpoint, {bool isColdStart = false}) async {
      final baseDelay = isColdStart ? 800 : 200;
      final randomDelay = Random().nextInt(300);

      await Future.delayed(Duration(milliseconds: baseDelay + randomDelay));

      return {
        'success': Random().nextDouble() > 0.02, // 98% 成功率
        'data': {
          'endpoint': endpoint,
    ...
    /// 模擬併發API調用
    Future<Map<String, dynamic>> _mockConcurrentApiCall(String endpoint, String userId) async {
      final delay = Random().nextInt(500) + 100;
      await Future.delayed(Duration(milliseconds: delay));

      return {
        'success': Random().nextDouble() > 0.03, // 97% 成功率
        'userId': userId,
        'endpoint': endpoint,
        'responseTime': delay,
      };
    ...
    /// 模擬用戶註冊
    Future<Map<String, dynamic>> _mockUserRegistration(String email) async {
      final delay = Random().nextInt(1000) + 200;
      await Future.delayed(Duration(milliseconds: delay));

      final isSuccess = Random().nextDouble() > 0.05; // 95% 成功率

      return {
        'success': isSuccess,
        'userId': isSuccess ? 'user_${Random().nextInt(999999)}' : null,
        'email': email,
        'timestamp': DateTime.now().toIso8601String(),
      };
    }
    ...
    /// 模擬資源鎖定操作（用於死鎖測試）
    Future<Map<String, dynamic>> _mockResourceLockingOperation(String resource1, String resource2) async {
      // 模擬獲取第一個資源
    ...
    /// 模擬負載
    Future<void> _simulateLoad(int concurrency, Duration duration) async {
      final endTime = DateTime.now().add(duration);

      while (DateTime.now().isBefore(endTime)) {
        final futures = <Future>[];
        for (int i = 0; i < concurrency; i++) {
          futures.add(_mockApiCall('loadTest'));
        }
    ...
    int _getMemoryUsage() {
      // 模擬記憶體使用量（bytes）
      return Random().nextInt(50 * 1024 * 1024) + 100 * 1024 * 1024; // 100-150MB
    }
    ...
    /// 模擬服務故障
    void _simulateServiceFailure() {
      // 設置故障狀態標誌
      _serviceFailureFlag = true;
    }

    /// 檢查是否檢測到故障
    bool _isFailureDetected() {
      return _serviceFailureFlag;
    }

    /// 觸發恢復程序
    Future<void> _triggerRecoveryProcedure() async {
      await Future.delayed(Duration(seconds: 1));
      _serviceFailureFlag = false;
    }

    /// 檢查服務健康狀態
    Future<Map<String, dynamic>> _checkServiceHealth() async {
      await Future.delayed(Duration(milliseconds: 500));

      return {
        'healthy': !_serviceFailureFlag,
        'responseTime': Random().nextInt(1000) + 500,
        'timestamp': DateTime.now().toIso8601String(),
      };
    }

    /// 模擬系統崩潰
    void _simulateSystemCrash() {
      _systemCrashFlag = true;
    }

    /// 創建測試用戶資料
    Future<List<Map<String, dynamic>>> _createTestUserData() async {
    ...
    /// 檢索測試用戶資料
    Future<List<Map<String, dynamic>>> _retrieveTestUserData() async {
      await Future.delayed(Duration(milliseconds: 200));

      // 模擬災難恢復後的資料檢索
      return await _createTestUserData();
    }

    /// 計算資料校驗和
    String _calculateDataChecksum(List<Map<String, dynamic>> data) {
      final jsonString = data.map((item) => item.toString()).join();
      return jsonString.hashCode.toString();
    }

    // 故障狀態標誌
    bool _serviceFailureFlag = false;
    bool _systemCrashFlag = false;

    // ================================================================
    // 安全性測試輔助方法 (用於TC-021, TC-022, TC-023, TC-024, TC-043)
    // ================================================================
    ...
    String _generatePasswordHash(String password) {
      // 模擬加鹽雜湊
      final salt = Random().nextInt(999999).toString();
    ...
    Future<Map<String, dynamic>> _attemptLogin(String email, String password) async {
      await Future.delayed(Duration(milliseconds: 100));
      final isSuccess = _validatePasswordStrength(password) && email == 'test@example.com';
      return {'success': isSuccess};
    }

    /// 生成JWT Token
    String _generateJWTToken(String userId, {String? deviceId}) {
      final payload = {'userId': userId, 'deviceId': deviceId, 'exp': DateTime.now().add(Duration(hours: 1)).millisecondsSinceEpoch};
      final header = {'alg': 'HS256', 'typ': 'JWT'};
      final encodedHeader = base64Url.encode(utf8.encode(jsonEncode(header)));
      final encodedPayload = base64Url.encode(utf8.encode(jsonEncode(payload)));
      final signature = 'signature_${encodedHeader}.${encodedPayload}';
      return '$encodedHeader.$encodedPayload.$signature';
    }

    /// 驗證Token簽名
    bool _verifyTokenSignature(String token) {
      return token.split('.').length == 3 && token.endsWith('signature_${token.split('.')[0]}.${token.split('.')[1]}');
    }

    /// 檢查Token是否過期
    bool _isTokenExpired(String token) {
      try {
        final payload = jsonDecode(utf8.decode(base64Url.decode(token.split('.')[1])));
        final exp = payload['exp'] as int;
        return DateTime.fromMillisecondsSinceEpoch(exp).isBefore(DateTime.now());
      } catch (_) {
        return true; // 無效Token視為過期
      }
    }

    /// 刷新Token
    String _refreshToken(String token) {
      return _generateJWTToken(token.split('.')[1]); // 模擬刷新
    }

    /// 綁定平台帳戶
    Future<Map<String, dynamic>> _bindPlatformAccount(String userId, String platform, String platformUserId) async {
      await Future.delayed(Duration(milliseconds: 300));
      return {'success': true, 'verified': true, 'userId': userId, 'platform': platform};
    }
  }
}
</new_str>
