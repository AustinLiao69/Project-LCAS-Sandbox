...
# 5005. TDD_SR_排程提醒模組_v1.2

## 文件資訊
- **文件標題**: SR 排程提醒模組技術設計文件
- **文件版本**: v1.2
- **創建日期**: 2025-07-19 15:30:00 +08:00 (台灣時間)
- **創建者**: LCAS SA Team
- **最後更新**: 2025-01-09 20:00:00 +08:00 (台灣時間)
...
3. 核心函數設計
   ...
   - 3.3 推播服務層函數
   - 3.4 數據整合層函數
   - 3.5 Quick Reply 專用層函數

4. Quick Reply 架構設計
   - 4.1 Quick Reply 技術實作架構
   - 4.2 WH-SR Quick Reply 協作協議
   - 4.3 付費功能在 Quick Reply 的整合
...
### 1.1 模組定位
SR 排程提醒模組作為 LCAS 2.0 的智慧記帳自動化核心，負責處理定期記帳提醒、自動記帳執行、財務統計推播、Quick Reply 互動等功能。本模組專注於排程邏輯和業務規則，透過其他模組提供的標準化介面進行資料庫操作和外部服務整合。
...
- **日誌系統**: DL 模組
- **資料分發**: DD 模組
- **Quick Reply 處理**: LINE Messaging API
 ...
 * SR_排程提醒模組_1.2.0
 * @module SR模組
 * @description 智慧記帳自動化核心 - 排程提醒、自動記帳、財務統計推播、Quick Reply 互動
 * @update 2025-01-09: 升級至v1.2版本，新增Quick Reply功能架構，擴充付費功能整合機制
 */
```
...
|---|----------|------|----------|----------|----------|
| 01 | SR_createScheduledReminder | V1.2.0 | 排程管理層 | 建立新的排程提醒設定 | AM、FS、DL |
| 02 | SR_updateScheduledReminder | V1.2.0 | 排程管理層 | 修改現有排程提醒設定 | FS、SR、DL |
| 03 | SR_deleteScheduledReminder | V1.2.0 | 排程管理層 | 安全刪除排程提醒 | FS、SR、DL |
| 04 | SR_executeScheduledTask | V1.2.0 | 排程管理層 | 執行到期的排程任務 | FS、WH、BK、DL |
| 05 | SR_processHolidayLogic | V1.2.0 | 排程管理層 | 處理國定假日邏輯 | FS、SR、DL |
| 06 | SR_optimizeReminderTime | V1.2.0 | 排程管理層 | 智慧時間最佳化 | AM、MRA、FS、DL |
| 07 | SR_validatePremiumFeature | V1.2.0 | 付費功能控制層 | 驗證付費功能權限 | AM、DL |
| 08 | SR_checkSubscriptionStatus | V1.2.0 | 付費功能控制層 | 檢查訂閱狀態 | FS、AM、DL |
| 09 | SR_enforceFreeUserLimits | V1.2.0 | 付費功能控制層 | 強制免費用戶限制 | FS、WH、DL |
| 10 | SR_upgradeFeatureAccess | V1.2.0 | 付費功能控制層 | 升級功能存取權限 | FS、AM、DD、DL |
| 11 | SR_sendDailyFinancialSummary | V1.2.0 | 推播服務層 | 發送每日財務摘要 | SR、MRA、WH、DL |
| 12 | SR_sendBudgetWarning | V1.2.0 | 推播服務層 | 發送預算警告 | BM、SR、WH、DL |
| 13 | SR_sendMonthlyReport | V1.2.0 | 推播服務層 | 發送月度報告 | MRA、SR、WH、DL |
| 14 | SR_processQuickReplyStatistics | V1.2.0 | 推播服務層 | 處理Quick Reply統計 | FS、DD、WH、DL |
| 15 | SR_syncWithAccountModule | V1.2.0 | 數據整合層 | 與AM模組同步 | AM、FS、DL |
| 16 | SR_syncWithDataDistribution | V1.2.0 | 數據整合層 | 與DD模組同步 | DD、FS、DL |
| 17 | SR_logScheduledActivity | V1.2.0 | 數據整合層 | 記錄排程活動 | DL、FS |
| 18 | SR_handleSchedulerError | V1.2.0 | 數據整合層 | 處理排程器錯誤 | DL、WH、SR |
| 19 | SR_handleQuickReplyInteraction | V1.2.0 | Quick Reply 專用層 | 統一處理Quick Reply互動 | WH、AM、DD、DL |
| 20 | SR_generateQuickReplyOptions | V1.2.0 | Quick Reply 專用層 | 動態生成Quick Reply選項 | AM、SR、DL |
| 21 | SR_handlePaywallQuickReply | V1.2.0 | Quick Reply 專用層 | 處理付費功能牆Quick Reply | AM、WH、SR、DL |
...
│   └── SR_processQuickReplyStatistics() - 處理Quick Reply統計（重大升級）
├── 數據整合層 (4個函數)
...
└── Quick Reply 專用層 (3個新增函數)
    ├── SR_handleQuickReplyInteraction() - 統一處理Quick Reply互動
    ├── SR_generateQuickReplyOptions() - 動態生成Quick Reply選項
    └── SR_handlePaywallQuickReply() - 處理付費功能牆Quick Reply
```
...
```javascript
/**
 * 14. 處理Quick Reply統計 - 重大功能擴充
 * @version 2025-01-09-V1.2.0
 * @date 2025-01-09 20:00:00
 * @update: 新增完整Quick Reply統計處理邏輯，支援三種統計類型和動態選項生成
 */
function SR_processQuickReplyStatistics(userId, statisticType, queryParams)
```
- **功能**: 全面升級的Quick Reply統計處理，支援今日、本週、本月統計，整合付費功能檢查
- **新增功能**: 
  - 支援三種統計類型的完整處理邏輯
  - 動態生成對應的Quick Reply選項
  - 整合付費功能檢查機制
  - 與DD模組深度整合進行數據處理
- **與其他模組互動**:
  ...
  - **DL模組** `DL_info()` - 記錄統計查詢
  - **AM模組** `AM_checkSubscriptionStatus()` - 檢查訂閱狀態（新增）
  - **SR模組** `SR_generateQuickReplyOptions()` - 生成動態選項（新增）
- **回傳值**: `{success: boolean, statisticData: object, quickReplyOptions: array, hasPaywall: boolean}`
...
```javascript
/**
 * 19. 統一處理Quick Reply互動事件
 * @version 2025-01-09-V1.2.0
 * @date 2025-01-09 20:00:00
 * @description 統一處理所有Quick Reply互動事件，包含路由分發和回應生成
 */
function SR_handleQuickReplyInteraction(userId, interactionType, interactionData, contextData)
```
- **功能**: 作為Quick Reply互動的中央處理器，負責路由分發和統一回應格式
- **核心邏輯**: 
  - 解析Quick Reply互動類型（統計查詢、付費功能、設定變更）
  - 根據用戶權限和上下文決定處理路徑
  - 統一生成符合LINE Messaging API格式的回應
  - 管理互動會話狀態和過期機制
- **與其他模組互動**:
  - **WH模組** `WH_sendQuickReply()` - 發送Quick Reply回應
  - **AM模組** `AM_getUserInfo()` - 取得使用者資訊
  - **DD模組** `DD_processDataQuery()` - 處理資料查詢
  - **DL模組** `DL_log()` - 記錄互動事件
  - **SR模組** `SR_generateQuickReplyOptions()` - 生成動態選項
- **回傳值**: `{success: boolean, responseData: object, quickReplyOptions: array, sessionId: string}`

```javascript
/**
 * 20. 動態生成Quick Reply選項
 * @version 2025-01-09-V1.2.0
 * @date 2025-01-09 20:00:00
...
```
- **功能**: 智慧化Quick Reply選項生成器，根據使用者狀態和權限動態調整可用選項
- **核心邏輯**: 
  - 分析使用者訂閱狀態和可用功能
  - 根據上下文類型生成適當的選項清單
  - 處理付費功能的升級提示選項
  - 生成符合LINE規格的Quick Reply按鈕配置
- **與其他模組互動**:
  - **AM模組** `AM_checkSubscriptionStatus()` - 檢查訂閱狀態
  - **SR模組** `SR_validatePremiumFeature()` - 驗證功能權限
  - **DL模組** `DL_info()` - 記錄選項生成
  - **FS模組** `FS_getUserPreferences()` - 取得使用者偏好設定
- **回傳值**: `{options: array, hasPaywall: boolean, contextData: object, maxOptions: number}`

```javascript
/**
 * 21. 處理付費功能牆Quick Reply互動
 * @version 2025-01-09-V1.2.0
 * @date 2025-01-09 20:00:00
 * @description 處理免費用戶觸發付費功能時的Quick Reply互動流程
 */
function SR_handlePaywallQuickReply(userId, blockedFeature, userContext, interactionData)
```
- **功能**: 專門處理付費功能牆的Quick Reply互動機制，提供友善的升級引導流程
- **核心邏輯**: 
  - 偵測免費用戶對付費功能的存取嘗試
  - 根據用戶的訂閱狀態決定是否顯示付費牆
  - 生成包含升級、試用、了解更多的Quick Reply選項
  - 記錄用戶對付費功能的興趣度供後續行銷參考
  - 與計費系統連動處理升級流程
- **與其他模組互動**:
  - **AM模組** `AM_checkSubscriptionStatus()` - 驗證用戶訂閱狀態
  - **WH模組** `WH_sendUpgradePrompt()` - 發送升級提示訊息
  - **SR模組** `SR_validatePremiumFeature()` - 檢查功能權限
  - **DL模組** `DL_warning()` - 記錄付費功能存取嘗試
  - **FS模組** `FS_createUserInteraction()` - 記錄使用者互動
- **回傳值**: `{upgradePrompt: boolean, trialOffered: boolean, redirectUrl: string, quickReplyOptions: array}`

---

## 4.0 Quick Reply 架構設計
...
```
Quick Reply 處理流程：
使用者點擊按鈕 → WH模組接收事件 → SR_handleQuickReplyInteraction() → 
根據類型路由 → 資料處理 → SR_generateQuickReplyOptions() → 回應生成 → 
WH模組發送回應 → 使用者接收
```

#### 4.1.2 Quick Reply 類型分類
```javascript
...
#### 4.3.1 免費功能 Quick Reply 配置
```javascript
const freeFeatureQuickReplies = {
  statistics: {
    today: {label: "今日統計", action: "today_stats", paywall: false},
    week: {label: "本週統計", action: "week_stats", paywall: false},
    month: {label: "本月統計", action: "month_stats", paywall: false}
  },
  basic_reminder: {
    create: {label: "新增提醒", action: "create_reminder", paywall: false, quota: true}
  }
};
```

#### 4.3.2 付費功能推廣 Quick Reply 配置
```javascript
const paywallQuickReplies = {
  upgrade_options: [
    {label: "立即升級", action: "upgrade", url: "payment_url", priority: 1},
    {label: "免費試用", action: "trial", paywall: false, priority: 2},
    {label: "了解更多", action: "learn_more", paywall: false, priority: 3}
  ],
  trial_conditions: {
    max_trial_days: 7,
...
### 5.2 資料流向設計
```
標準排程觸發 → SR排程驗證 → 付費功能檢查 → 條件判斷 → 
執行動作 → 結果處理 → 跨模組同步 → 日誌記錄 → 完成回應

Quick Reply處理 → WH事件接收 → SR_handleQuickReplyInteraction() → 
類型判斷 → 權限檢查 → 資料處理 → SR_generateQuickReplyOptions() → 
回應生成 → WH發送回應 → 使用者接收
```
...
```
quick_reply_sessions/ (透過FS模組管理)
├── {session_id}/
│   ├── userId: string (索引)
│   ├── sessionType: "statistics" | "paywall" | "settings"
│   ├── contextData: object
...
│   │   ├── paywall: boolean
│   │   └── priority: number
│   ├── lastInteraction: timestamp (UTC+8)
│   ├── expiryTime: timestamp (UTC+8)
│   ├── isActive: boolean
│   └── metadata: object

user_interactions/ (透過FS模組管理)
├── {interaction_id}/
│   ├── userId: string (索引)
│   ├── interactionType: "quick_reply" | "paywall" | "upgrade"
│   ├── featureName: string
│   ├── actionTaken: string
│   ├── subscriptionLevel: "free" | "premium"
...
POST   /api/v2/scheduler/quickreply/handle    - 處理Quick Reply互動（新增）
GET    /api/v2/scheduler/quickreply/options   - 取得Quick Reply選項（新增）
POST   /api/v2/scheduler/paywall/interact     - 處理付費功能牆互動（新增）
```
...
const ErrorTypes = {
  ...
  QUICK_REPLY_SESSION_EXPIRED: 'qr_session_expired', // 新增
  QUICK_REPLY_INVALID_OPTION: 'qr_invalid_option', // 新增
  PAYWALL_BLOCKED: 'paywall_blocked', // 新增
...
async function SR_monitorSystemHealth() {
  const healthMetrics = {
    ...
    quickReplySessions: await FS_countActiveQuickReplySessions(), // 新增
    paywallInteractions: await FS_countPaywallInteractions(), // 新增
  ...
  const isHealthy = healthMetrics.failedExecutions < 10 && 
                   ...
                   healthMetrics.quickReplySessions < 1000; // 新增檢查
...
const cacheConfig = {
  ...
  quickReplyOptions: new NodeCache({ stdTTL: 180 }), // 3分鐘（新增）
  paywallConfig: new NodeCache({ stdTTL: 1800 }) // 30分鐘（新增）
};
```
...
- SR_handleQuickReplyInteraction() 測試（新增）
- SR_generateQuickReplyOptions() 測試（新增）
- SR_handlePaywallQuickReply() 測試（新增）
- 錯誤處理機制測試
...
- 排程執行準確性測試
- 資料庫一致性測試
- Quick Reply 互動流程測試（新增）
- 付費功能牆測試（新增）

### 12.3 效能測試指標
- 排程執行延遲 < 30秒
- 並發處理能力 > 1000個排程
- Quick Reply 回應時間 < 2秒（新增）
- 記憶體使用率 < 80%
- 錯誤率 < 1%
...
| v1.1 | 2025-01-09 16:00:00 +08:00 | LCAS SA Team | 升級版本，新增完整目次和函數清單表格，優化文件結構和導航，所有函數版本升級至V1.1.0 |
| v1.2 | 2025-01-09 20:00:00 +08:00 | LCAS SA Team | 重大版本升級，新增Quick Reply專用層3個函數（19-21），重大升級函數14，新增Quick Reply架構設計章節，完整付費功能整合機制，所有函數版本升級至V1.2.0 |

---

**文件結束**
*此文件為 LCAS 2.0 SR 排程提醒模組的完整技術設計規格 v1.2 版本，包含完整目次、函數清單表格、21個核心函數的詳細設計和完整的Quick Reply架構實作，所有函數設計均基於 PRD 需求並遵循模組間協作原則。*
