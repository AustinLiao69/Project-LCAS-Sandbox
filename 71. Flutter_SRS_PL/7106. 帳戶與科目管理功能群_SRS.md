...
**核心整合點**：
- **錢包管理**：7106 → APL.account → ASL.js → WCM_createWallet、WCM_getWalletList等函數
...
|-------------|---------------------|------------|------------|
| 錢包資料管理 | APL.account服務 | `/api/v1/accounts` | WCM_createWallet, WCM_getWalletList |
| 科目階層管理 | APL.category服務（P3預留） | `/api/v1/categories` | WCM_createCategory, WCM_getCategoryList |
| 錢包餘額計算 | APL.account服務 | `/api/v1/accounts/{id}/balance` | WCM_getWalletBalance |
| 錢包驗證邏輯 | APL.account服務 | `/api/v1/accounts/{id}` | WCM_validateWalletExists |
...
class AccountListData {
  ...
  AccountStatistics? statistics;  // 統計資料
