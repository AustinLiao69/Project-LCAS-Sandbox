...
**47個測試案例**
- **TC-SIT-001~016**：BL層測試（16個）
- **TC-SIT-017~047**：APL層測試（31個）
...
**TC-SIT-045：/api/v1/system/health 端點完整驗證**
- **測試焦點**: 系統健康檢查功能
- **API端點**: 8111系統服務
- **測試步驟**:
  1. 發送GET請求到/api/v1/system/health端點
  2. 驗證健康檢查回應格式
  3. 檢查各服務狀態資訊
  4. 驗證效能指標準確性
  5. 確認監控告警機制
...
**TC-SIT-046：/api/v1/system/app-info 端點完整驗證**
- **測試焦點**: APP資訊查詢功能
- **API端點**: 8111系統服務
- **測試步驟**:
  1. 發送GET請求到/api/v1/system/app-info端點
  2. 驗證APP版本資訊
  3. 檢查功能特色說明
  4. 驗證更新資訊準確性
  5. 確認四模式差異化資訊
...
**TC-SIT-047：/api/v1/system/welcome 端點完整驗證**
- **測試焦點**: 歡迎頁面資訊功能
- **API端點**: 8111系統服務
- **測試步驟**:
  1. 發送GET請求到/api/v1/system/welcome端點
  2. 驗證歡迎內容格式
  3. 檢查引導步驟資訊
  4. 驗證推廣內容準確性
  5. 確認四模式差異化歡迎內容
...
#### 新增API契約測試（TC-SIT-026~047）- 擴展測試覆蓋
| 測試案例ID | 測試案例名稱 | API端點 | 測試焦點 |
|------------|--------------|---------|----------|
| TC-SIT-026 | /api/v1/auth/refresh 端點完整驗證 | 8101認證服務 | Token刷新機制驗證 |
| TC-SIT-027 | /api/v1/auth/forgot-password 端點完整驗證 | 8101認證服務 | 密碼重設流程驗證 |
| TC-SIT-028 | /api/v1/auth/reset-password 端點完整驗證 | 8101認證服務 | 密碼重設執行驗證 |
| TC-SIT-029 | /api/v1/auth/verify-email 端點完整驗證 | 8101認證服務 | Email驗證流程 |
| TC-SIT-030 | /api/v1/auth/bind-line 端點完整驗證 | 8101認證服務 | LINE帳號綁定功能 |
| TC-SIT-031 | /api/v1/auth/bind-status 端點完整驗證 | 8101認證服務 | 帳號綁定狀態查詢 |
| TC-SIT-032 | GET /api/v1/users/profile 端點完整驗證 | 8102用戶管理服務 | 用戶資料查詢功能 |
| TC-SIT-033 | PUT /api/v1/users/profile 端點完整驗證 | 8102用戶管理服務 | 用戶資料更新功能 |
| TC-SIT-034 | /api/v1/users/preferences 端點完整驗證 | 8102用戶管理服務 | 用戶偏好設定管理 |
| TC-SIT-035 | /api/v1/users/mode 端點完整驗證 | 8102用戶管理服務 | 用戶模式切換功能 |
| TC-SIT-036 | /api/v1/users/security 端點完整驗證 | 8102用戶管理服務 | 安全設定管理 |
| TC-SIT-037 | /api/v1/users/verify-pin 端點完整驗證 | 8102用戶管理服務 | PIN碼驗證功能 |
| TC-SIT-038 | GET /api/v1/transactions/{id} 端點完整驗證 | 8103記帳交易服務 | 單筆交易查詢功能 |
| TC-SIT-039 | PUT /api/v1/transactions/{id} 端點完整驗證 | 8103記帳交易服務 | 交易記錄更新功能 |
| TC-SIT-040 | DELETE /api/v1/transactions/{id} 端點完整驗證 | 8103記帳交易服務 | 交易記錄刪除功能 |
| TC-SIT-041 | /api/v1/transactions/statistics 端點完整驗證 | 8103記帳交易服務 | 交易統計分析功能 |
| TC-SIT-042 | /api/v1/transactions/recent 端點完整驗證 | 8103記帳交易服務 | 最近交易查詢功能 |
| TC-SIT-043 | /api/v1/transactions/charts 端點完整驗證 | 8103記帳交易服務 | 交易圖表資料功能 |
| TC-SIT-044 | /api/v1/transactions/dashboard 端點完整驗證 | 8103記帳交易服務 | 儀表板綜合資料 |
| TC-SIT-045 | /api/v1/system/health 端點完整驗證 | 8111系統服務 | 系統健康檢查功能 |
| TC-SIT-046 | /api/v1/system/app-info 端點完整驗證 | 8111系統服務 | APP資訊查詢功能 |
| TC-SIT-047 | /api/v1/system/welcome 端點完整驗證 | 8111系統服務 | 歡迎頁面資訊功能 |