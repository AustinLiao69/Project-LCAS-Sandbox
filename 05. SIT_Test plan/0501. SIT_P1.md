...
**最後更新：** 2025-10-03  
**負責人：** LCAS SQA Team  
**適用範圍：** Phase 1 核心進入流程系統整合測試 - MVP精簡版
...
### 3.2 階段二：API契約層測試 - TC-SIT-017~025
...
- **驗證重點**: 業務邏輯正確性
- **測試步驟**: PL層函數呼叫→APL層驗證→BL層AM模組處理→DL層Firestore儲存→結果回傳
- **預期結果**: 註冊流程100%成功，無資料遺失

**TC-SIT-002：登入驗證整合測試**  
- **測試焦點**: 端到端流程驗證
- **驗證重點**: 業務邏輯正確性
- **驗證點**: 正確憑證成功登入、錯誤憑證適當錯誤、JWT Token格式正確、四模式資訊返回

**TC-SIT-003：Firebase Auth整合測試**
- **測試焦點**: 業務邏輯正確性
- **驗證重點**: Firebase服務整合完整性
- **測試場景**: Firebase ID Token驗證、用戶註冊流程、登入認證流程、Token過期處理
...
- **測試焦點**: 完整使用者體驗差異
- **驗證重點**: 四模式差異化處理
- **驗證點**: 文字解析準確性、記帳記錄正確儲存、回應訊息正確格式

**TC-SIT-005：完整記帳表單整合測試**
- **測試焦點**: 跨層整合流程
- **驗證重點**: 端到端流程驗證
- **驗證點**: 表單驗證正確執行、附件處理功能正常、資料完整性保證

**TC-SIT-006：記帳資料查詢整合測試**
- **測試焦點**: 端到端流程驗證
- **驗證重點**: 跨層整合流程
- **驗證點**: 資料查詢準確性、四模式回應差異化、查詢結果完整性
...
- **測試焦點**: 跨層整合流程
- **驗證重點**: 錯誤處理覆蓋率60%
- **測試場景**: 網路超時、Firebase認證錯誤、認證過期、資料驗證錯誤
- **驗證點**: 錯誤正確傳遞、訊息格式統一、使用者友善提示、適當恢復機制

**TC-SIT-008：模式評估整合測試**
- **測試焦點**: 四模式差異化
- **驗證重點**: 完整使用者體驗差異
- **驗證點**: 評估邏輯正確執行、模式分配準確、用戶設定正確更新

**TC-SIT-009：模式差異化回應測試**
- **測試焦點**: 完整使用者體驗差異
- **驗證重點**: 四模式差異化實現
- **測試場景**: Expert模式詳細回應、Guiding模式簡化回應、Inertial模式一致性回應、Cultivation模式激勵性回應

**TC-SIT-010：資料格式轉換測試**
- **測試焦點**: 跨層整合流程
- **驗證重點**: 資料一致性保證
- **驗證點**: 格式轉換準確性、資料完整性、類型正確性

**TC-SIT-011：資料同步機制測試**
- **測試焦點**: PL→APL→BL→DL完整鏈路
- **驗證重點**: 資料同步準確性
- **驗證點**: 同步時效性、資料一致性、衝突處理機制
...
- **測試焦點**: 端到端流程驗證
- **驗證重點**: 業務價值鏈完整性
- **測試流程**: 註冊→登入→模式評估→記帳操作→資料查詢→登出

**TC-SIT-013：記帳業務流程端到端測試**
- **測試焦點**: 業務邏輯正確性
- **驗證重點**: 記帳核心功能完整性
- **測試流程**: 快速記帳→完整表單記帳→資料查詢→統計分析

**TC-SIT-014：網路異常處理測試**
- **測試焦點**: 跨層錯誤處理
- **驗證重點**: 異常情況下的系統穩定性
- **測試場景**: 網路中斷、請求超時、服務暫時不可用

**TC-SIT-015：業務規則錯誤處理測試**
- **測試焦點**: 業務邏輯正確性
- **驗證重點**: 業務規則驗證準確性
- **測試場景**: 無效資料輸入、業務規則衝突、權限不足

**TC-SIT-016：DCN-0015格式驗證測試**
- **測試焦點**: API回應格式標準化
- **驗證重點**: 統一回應格式100%合規
- **驗證架構**: 三層驗證（基礎結構→深度內容→業務邏輯）
...
| 測試案例ID | 測試案例名稱 | 測試焦點 | 優先級 | 預估時間 |
|------------|--------------|----------|--------|----------|
| TC-SIT-001 | 使用者註冊流程整合測試 | PL→APL→BL→DL完整鏈路 | P1 | 4小時 |
| TC-SIT-002 | 登入驗證整合測試 | 端到端流程驗證 | P1 | 3小時 |
| TC-SIT-003 | Firebase Auth整合測試 | 業務邏輯正確性 | P1 | 3小時 |
| TC-SIT-004 | 快速記帳整合測試 | 完整使用者體驗差異 | P1 | 4小時 |
| TC-SIT-005 | 完整記帳表單整合測試 | 跨層整合流程 | P1 | 4小時 |
| TC-SIT-006 | 記帳資料查詢整合測試 | 端到端流程驗證 | P1 | 3小時 |
| TC-SIT-007 | 跨層錯誤處理整合測試 | 跨層整合流程 | P1 | 5小時 |
| TC-SIT-008 | 模式評估整合測試 | 四模式差異化 | P1 | 4小時 |
| TC-SIT-009 | 模式差異化回應測試 | 完整使用者體驗差異 | P1 | 6小時 |
| TC-SIT-010 | 資料格式轉換測試 | 跨層整合流程 | P2 | 3小時 |
| TC-SIT-011 | 資料同步機制測試 | PL→APL→BL→DL完整鏈路 | P2 | 3小時 |
| TC-SIT-012 | 使用者完整生命週期測試 | 端到端流程驗證 | P1 | 5小時 |
| TC-SIT-013 | 記帳業務流程端到端測試 | 業務邏輯正確性 | P1 | 4小時 |
| TC-SIT-014 | 網路異常處理測試 | 跨層錯誤處理 | P2 | 3小時 |
| TC-SIT-015 | 業務規則錯誤處理測試 | 業務邏輯正確性 | P2 | 3小時 |
| TC-SIT-016 | DCN-0015格式驗證測試 | API回應格式標準化 | P1 | 4小時 |

#### API契約測試（TC-SIT-017~025）- 驗證技術規格合規
| 測試案例ID | 測試案例名稱 | API端點 | 測試焦點 | 預估時間 |
|------------|--------------|---------|----------|----------|
| TC-SIT-017 | /auth/register 端點完整驗證 | 8101認證服務 | API規格合規性 | 3小時 |
| TC-SIT-018 | /auth/login 端點完整驗證 | 8101認證服務 | API契約驗證 | 3小時 |
| TC-SIT-019 | /auth/logout 端點完整驗證 | 8101認證服務 | API回應欄位差異 | 2小時 |
| TC-SIT-020 | /users/profile 端點完整驗證 | 8102用戶管理服務 | 單一API端點 | 3小時 |
| TC-SIT-021 | /users/assessment 端點完整驗證 | 8102用戶管理服務 | APL層API規格合規性 | 3小時 |
| TC-SIT-022 | /users/preferences 端點完整驗證 | 8102用戶管理服務 | API契約驗證 | 2小時 |
| TC-SIT-023 | /transactions/quick 端點完整驗證 | 8103記帳交易服務 | API規格合規性 | 3小時 |
| TC-SIT-024 | /transactions CRUD端點完整驗證 | 8103記帳交易服務 | 單一API端點 | 4小時 |
| TC-SIT-025 | /transactions/dashboard 端點完整驗證 | 8103記帳交易服務 | API回應格式標準化 | 3小時 |